      SUBROUTINE SNLSGU(N, P, L, ALF, C, Y, SCALCA, SCALCB, INC, IINC,
     1                  IV, LIV, LV, V)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1996-04-27 SNLSGU Krogh  Changes to get desired C prototypes.
C>> 1994-10-20 SNLSGU Krogh  Changes to use M77CON
C>> 1990-07-02 SNLSGU CLL @ JPL
C>> 1990-06-12 CLL @ JPL
C>> 1990-02-20 CLL @ JPL
c--S replaces "?": ?NLSGU,?CALCA,?CALCB,?IVSET,?RNSG
C
...
C T(1)...T(N),   SNLSGU ATTEMPTS TO COMPUTE A LEAST SQUARES FIT
...
C                      OF IV AND V BEFORE CALLING   SNLSGU.
C
...
C
      EXTERNAL SIVSET,  SRNSG
C
C SIVSET.... PROVIDES DEFAULT IV AND V VALUES.
C  SRNSG... CARRIES OUT NL2SOL ALGORITHM.
C
C  ***  LOCAL VARIABLES  ***
C
...
C
 50   IV(1) = 66
C
 60   CALL  SRNSG(V, ALF, C, V, IV, IV, L, 1, N, LIV, LV, N, M, P, V, Y)
...
C
 90   A1 = IV(AMAT)
      ...
      M = IV(MSAVE)
C
 100  CALL  SRNSG(V(A1), ALF, C, V(DA1), IV(IN1), IV, L, L1, N, LIV, LV,
     1            N, M, P, V, Y)
