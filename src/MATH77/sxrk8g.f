      subroutine SXRK8G (TS, Y, WORK, IDAT)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 2008-02-25 SXRK8G Krogh Lots of changes in usage, bug fixes.
c>> 1997-02-18 SXRK8G  Krogh  Initial code.
c--S replaces "?": ?XRK8G, ?XRK8I, ?XRK8, ?MESS, ?ZERO

c Checks and finds G-Stops for the integrator SXRK8.
c   From dxrk8f:  call sxrk8g(TS, Y, F, IDAT)
c   Reverse Comm: call sxrk8g(TS, Y, WORK, IDAT)
c
...
c
c    Most names are defined in SXRK8.  Just those local to SXRK8G here.
c
c LI    IDAT(LOCINT) = Location in WORK where information connected with
c    interpolation is saved.  See IDAT(LOCINT) in SXRK8.
c LG    Difference between indexes for g and the old g.
c NG    Index for current G.
c TL    Value of T where G's were last computed on call to SZERO.
c
...
cAA SXRK8G$B
cAB Calling SXRK8G when IDAT(1) is not 1 or 2, and no G-Stop was $C
...
c   is at best questionable, and is certainly of low accuracy.$E
      ...
      data MTXTAA/'SXRK8G$BCalling SXRK8G when IDAT(1) is not 1 or 2, an
...
c Apparently there were multiple stops athe same value of t, get next.
          do 40 NG = IDAT(2) - 1, IDAT(3), -1
            if ((TS(NG)*TS(NG+LG).le.0.E0).and.(TS(NG+LG).ne.0.E0)) then
              IDAT(2) = NG
              IDAT(NGHI) = NG
              TS(NG+LG) = 0.E0
              return
...
c User just computed a full set of G's.  Set flag assuming no stop.
      ...
      if (abs(TS(1) - TS(LG+3)) .le. .25E0 * abs(TS(2))) then
...
c  (This is prevent noise from resulting in multiple G-Stops.)
        do 60 NG = NG, LG + 2
          if (TS(NG+LG) .ne. 0) then
            if (TS(NG) * TS(NG+LG) .le. 0.E0) then
              if (TS(NG) .ne. 0.E0) go to 300
            end if
            TS(NG+LG) = TS(NG)
          end if
 60     continue
...
c  sign change
      do 80 NG = NG, LG + 2
        if (TS(NG) * TS(NG+LG) .le. 0.E0) then
          if (TS(NG+LG) .ne. 0.E0) go to 300
        end if
        TS(NG+LG) = TS(NG)
 80   continue
 100  if (IDAT(3) .eq. 0.E0) then
...
c                       In the middle of an iteration
 320  NG = IDAT(3)
      ...
      NG = -NG
c## Note SZERO has save variables as does MESS and SMESS.
c At some point we need to set aside space in WORK for all saves that
c might be required.  In SZERO this is a good size set: DIV, FL, FLMFB,
c FO, KNKP, KS, KTYP, LCHG, LMODE, LNLP, MACT, NP, RND, SMALL, XL,
c XLMXB, XO, XX, XXMXOL.  (Even more in MESS and SMESS!) A simpler code
...
c
 350  TL = TS(1)
      call SZERO(TS(1),TS(NG),WORK(LI-5),WORK(LI-6),IDAT(KDZERO),0.E0)
...
c                          Not ready to interpolate
 ...
 380  call SXRK8I(TS(1), Y, IDAT, WORK)
      return
c                  Some kind of convergence
 400  if (TS(2) * (WORK(LI-5) - TS(1)) .gt. 0.E0) TS(1) = WORK(LI-5)
      IDAT(3) = NG
...
c Process an error.
 500  I = 1
 510  DATERR(1) = TS(1)
      call SMESS(MACT, MTXTAA, IDAT(I), DATERR)
      return
      end
