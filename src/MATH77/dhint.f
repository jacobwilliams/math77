      double precision function DHINT (X,NDERIV,NTAB,XTAB,YTAB,YPTAB)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 1994-11-11 DHINT  Krogh   Declared all vars.
C>> 1994-10-20 DHINT  Krogh  Changes to use M77CON
C>> 1987-12-09 DHINT  Lawson  Initial code.
c--D replaces "?": ?HINT
c
...
c     ------------------------------------------------------------------
      ...
      double precision ONE, TWO, THREE, SIX, Q0, Q1, Q2, Q3, Q4, Q5,R,X
...
C
  100 Q4=TWO*Q0
      ...
      Q4=Q1**2
      Q5=Q0**2
      DHINT  =Q4*(Q2*YTAB(LOOK)+R*Q0*YPTAB(LOOK))+
     *        Q5*(Q3*YTAB(LOOK1)+R*Q1*YPTAB(LOOK1))
      return
C
  101 Q2=THREE*Q0
      Q3=SIX*Q0/R
      DHINT  =Q1*(Q3*(YTAB(LOOK)-YTAB(LOOK1))+
     *        (Q2-ONE)*YPTAB(LOOK)) + Q0*(Q2-TWO)*YPTAB(LOOK1)
      return
c
  102 Q2 = Q1 + Q0
      DHINT = (TWO/R) * ( (THREE/R) * (YTAB(LOOK)-YTAB(LOOK1)) * Q2 +
     *        YPTAB(LOOK)*(Q2+Q1) + YPTAB(LOOK1)*(Q2+Q0) )

      return
c
  103 DHINT = (SIX/R**2) * ( (TWO/R) * (YTAB(LOOK)-YTAB(LOOK1)) +
