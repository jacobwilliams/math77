c  File: drDTGrec.[f|for] contains drDTGrec, DTGfcn.
      program DRDTGREC
c>> 2001-07-16 DRDTGREC Krogh Added exponent 0 to some constants.
c>> 2001-05-24 DRDTGREC Krogh Minor change for making .f90 version.
c>> 2001-05-22 DRDTGREC Krogh Minor change for making .f90 version.
c>> 1997-07-01 DRDTGREC Krogh Reversed subscripts in B (CLL suggestion)
c>> 1997-06-19 DRDTGREC Krogh Minor changes to get proper C conversion.
c>> 1997-06-18 DRDTGREC CLL Added Bdry and MB to arg list of ?TGREC.
c              Also ZFILL has extended interpretation.
c>> 1996-03-04 DRDTGREC CLL
c>> 1996-02-02 DRDTGREC CLL
c>> 1995-09-26 DRDTGREC CLL Editing for inclusion into MATH77.
c>> 1991-11-20 DRDTGREC CLL  Minor editing. Adding use of ?SORTP.
...
c          grid.
c  PRTT  Call DTGPRG to print the initial X() and Y() data, the
...
c
c     DRDTGREC           DEMONSTRATION DRIVER.
c        DTGGRD          CONSTRUCT TRIANGULAR GRID
c           ?SORTP       Sorting.   From the JPL Math77 library.
c           DTGANG       COMPUTE PSEUDOANGLES
c           ?TRSET       **     THESE FOUR SUBRS ARE USED TO STORE AND
c           DTGGET       **     FETCH POINTERS IN THE ARRAY TRIANG()
...
c             DTGGET
c             ?TRPUT
c        DTGPRG          print DESCRIPTION OF TRIANGULAR GRID
c           DTGGET
c        DTGFCN          PROVIDE VALUES OF TEST functionS.
c        DTGPD           ESTIMATE PARTIAL DERIVATIVES AT GRID POINTS
c           DTGGET
...
c        DTGREC          BUILD RECTANGULAR GRID OF INTERPOLATED VALUES.
...
c     double precision ZGRD(MX,MY), DZGRD(MX,MY,2)
c     double precision X(MP),Y(MP),Z(MP),DZ(2,MP),W(MP),XYLIM(4)
c     double precision DZZ(2),ERRDZ(2)
c     double precision DZTRUE(2,MP)
...
c     WHICH VALUES WILL BE INTERPOLATED UNDER CONTROL OF DTGREC.
c     ------------------------------------------------------------------
c--D replaces "?": DR?TGREC, ?TGGRD, ?TGPRG, ?TGFCN, ?TGPD, ?TGREC
c--&               ?TGGET,   ?TGANG
c     ------------------------------------------------------------------
      ...
      double precision delx, dely, DZ(2,mp), DZGRD(mx,my,2)
      double precision DZTRUE(2,mp), DZZ(2)
      double precision err, ERRDZ(2), errm, ERRMDZ(2)
      double precision rms, RMSDZ(2), sum1, SUM2(2)
      double precision W(mp), X(mp), xx, XYLIM(4),Y(mp), yy
      double precision Z(mp), zero, zfill, ZGRD(mx,my), zz
     ...
     *-0.88827D0,-0.60000D0,-0.88620D0, 0.08600D0 /
      data XYLIM / -1.0d0, +1.0d0, -1.0d0, +1.0d0 /
      data np, nx, ny / mp, mx, my /
      data wantpd / .true. /
c     ------------------------------------------------------------------
      write(*,'(a/a/)') ' Program DRDTGREC.  Demo driver for DTGREC,',
     * '         DTGGRD, DTGPD, etc.'
     ...
     *      ' Error return from DTGGRD. INFO(1) =', INFO(1)
      ...
      if(PRTT) call DTGPRG(X,Y,NP,TRIANG,Bdry,INFO(2),NT)
c
      do 200 KF= 0, 4
         do 20 I=1,NP
               call DTGFCN(KF,X(I),Y(I),
   ...
   20    continue
     ...
     *   line1,line2,   title,   line1,line2
         ...
         if (PRTTPD) then
            write(*,'(/a,a//a,a/41x,a/)')
     *      ' PARTIAL DERIVS ESTIMATED BY DTGPD AT THE GIVEN',
     *      ' DATA POINTS',
     *      '    I       X       Y       Z            ',
     *      'DZ1        DZTRUE1    ERR1',
     *      'DZ2        DZTRUE2    ERR2'
...
c
         ...
         call DTGREC(X,Y,Z,DZ,NP,TRIANG,NT, Bdry, MB, XYLIM,NX,NY,
...
c
     ...
     *       '  DZ2_INTERP   DZ2_TRUE   DZ2_ERR'
            ...
            do 80 I=1,NX
               YY = XYLIM(3)
               do 70 J=1,NY
                  call DTGFCN(KF,  XX,YY, title, ZZ, DZZ(1), DZZ(2) )
