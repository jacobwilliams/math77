c     program DRDNLSFU
c>> 2001-05-24 DRDNLSFU Krogh Minor change for making .f90 version.
c>> 1994-11-02 DRDNLSFU Krogh  Changes to use M77CON
c>> 1994-09-14 DRDNLSFU CLL Set IV(OUTLEV) = 0 for comparing output.
...
c     Demo driver for DNLSFU. A variant of the nonlinear LS code NL2SOL.
c     DNLSFU solves the "separable" problem.
c     DNLSFU requires function values only.
...
c     ------------------------------------------------------------------
c--D replaces "?": DR?NLSFU, ?NLSFU, ?CALCA, ?IVSET
c     ------------------------------------------------------------------
      external DCALCA
...
c     ------------------------------------------------------------------
      ...
      print'(1x,a)',
     * 'Program DRDNLSFU.. Demo driver for DNLSFU.',
     * '   A variant of NL2SOL.',
     * '   DNLSFU handles the Separable problem.',
     * '   DNLSFU requires function values but not the Jacobian.',
     * ' ',
     ...
     * ' '
      ...
      call DNLSFU(NDATA, NA, NB, ALF, BET, YDATA, DCALCA,
