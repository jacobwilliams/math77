      program DRSJACG1
c>>  2006-04-12 DRSJACG1 Hanson -- Reduced lengths of sjacg work arrays.
c>>  2003-07-08 DRSJACG1 Hanson -- Check for MODE < 0.
c>>  2003-07-07 DRSJACG1 Krogh -- Changed 3 arg max to 2 arg for C conv.
c>>  2002-06-21 DRSJACG1 R. J. Hanson Example 1 Code, with Download
c     Demo driver for SJACG, using numerical derivatives for a gradient.
c     ------------------------------------------------------------------
c--S  D replaces "?": DR?JACG1, ?JACG
...
C     Define sizes and parameters.
      ...
      REAL             Y(N), F(M), FJAC(LDFJAC,N), XSCALE(N),
     .     FAC(N), WK(LWK), ACTUAL(LDFJAC,N), RE(4,2)
      REAL             A, B, C, F2, U, MAXERR, R1MACH
      CHARACTER*55 WHAT(4)
      CHARACTER*1 uv
      DATA WHAT /
...
C      This sets the function value used in forming one-sided difference
      IF(MODE .eq. 0) THEN
         F(1)=WK(1)
      END IF
      CALL SJACG(MODE,M,N,Y,F,
