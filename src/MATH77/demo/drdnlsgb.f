c     program DRDNLSGB
c>> 2001-05-24 DRDNLSGB Krogh Minor change for making .f90 version.
c>> 1997-06-18 DRDNLSGB Krogh  Changes to improve C portability.
c>> 1994-11-02 DRDNLSGB Krogh  Changes to use M77CON
c>> 1994-09-14 DRDNLSGB CLL Set IV(OUTLEV) = 0 for comparing output.
...
c     Demo driver for DNLSGB. A variant of the nonlinear LS code NL2SOL.
c     DNLSGB solves the "separable" problem.
c     DNLSGB handles bounds on the nonlinear variables.
c     DNLSGB requires values of the function and the Jacobian matrix.
c     Note:  MDER is the number of ones in the array IND().
c     ------------------------------------------------------------------
c--D replaces "?": DR?NLSGB, ?NLSGB, ?CALCA, ?CALCB, ?IVSET, ?KEY
c     ------------------------------------------------------------------
      external DCALCA, DCALCB
      integer  J,IVAR,ITERM,LIV,LV,MA,MB,MDATA,MDER,NA,NB,NDATA
      parameter(MDATA = 30, MA = 2, MB = 5)
      parameter(MDER = 4)
      parameter(LIV = 115 + 4*MA + MB + 2*MDER)
      parameter(LV = 105 + MDATA*(MB+MDER+MA+3) +
     *               (MB*(MB+3))/2 + MA*(2*MA+21))
...
c     ------------------------------------------------------------------
      ...
      print'(1x,a)',
     * 'Program DRDNLSGB.. Demo driver for DNLSGB.',
     * '   A variant of NL2SOL.',
     ...
     * ' '
      ...
      call DNLSGB(NDATA, NA, NB, ALF, BND, BET, YDATA, DCALCA, DCALCB,
