      PROGRAM drddasl6
c>> 2009-10-21 DRDDASL6 Krogh at do 120 upper limit km => km+1.
c>> 2008-10-26 DRDDASL6 Krogh Moved Format statements up for C conv.
c>> 2008-10-24 DRDDASL6 Krogh Removed an INCLUDE statement & cDEC$...
c>> 2008-08-27 DRDDASL6  assign initial y' value, change user codes
c>> 2001-10-11 DR DDASL6  R. J. Hanson Example 6 Code, with Download
...
c--D replaces "?": ?DASLX, ?DASSF6, DR?DASL6
      EXTERNAL ddassf6
      INTEGER ndig
...
c Note that the N here and in routine DDASSF6 must have the same value.
...
c     evaluation program ?DASF (= ?DASF6).
      ...
      DOUBLE PRECISION t,u(n,n),uprime(n,n),rtol(neq),atol(neq),
...
c Partials and solutions are done in DDASSF6 below.
      info(5)=5
...
c initial call to DDASSF6.
  
c Set output points for return of solution.
      tout(1)=0.d0
      DO 30 i=2,11
        tout(i)=tout(i-1)+1.d0
   30 CONTINUE
  
      DO 40 i=2,11
        t=tout(i-1)
        CALL ddaslx (ddassf6, neq, t, u, uprime, tout(i), info, rtol,
...
c Summarize results and amount of work requested.
      ...
      SUBROUTINE ddassf6 (t, u, upr, del, d, ldd, cj, ires, rwork,
     & iwork)
c 
...
c     sides implicitly. Other functions are left to the DDASLX.
...
c here.
      ...
      DOUBLE PRECISION t,u(n,*),upr(n,*),del(n,*),d(ldd,*),cj,rwork(*),
     & u0(n,n),reltol,lambda,hsq3,hx,cc,ss,hsqlam,nrm,rss,rnrm,tol,tt
  
      DOUBLE PRECISION rr(0:n+1,0:n+1),xd(n,n)
      DOUBLE PRECISION hh(0:km,km+1),qq(n,n,km),ww(n,n)
      DOUBLE PRECISION scl(n,n)
      DOUBLE PRECISION cs(km),sn(km),y(km)
...
c (Automatic differentiation can start here.)
        DO 50 j=1,n
          DO 40 i=1,n
            del(i,j)=hsqlam*exp(u(i,j))-hsq3*upr(i,j)-8.d0*u(i,j)
            rr(i,j)=u(i,j)
   40     CONTINUE
   50   CONTINUE
...
c We precompute quantities that are expensive.
        ...
        DO 90 j=1,n
          DO 80 i=1,n
            u0(i,j)=hsqlam*exp(u(i,j))-hsq3*cj-8.d0
