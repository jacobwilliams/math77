c     program DRSNLAGB
c>> 2001-05-24 DRSNLAGB Krogh Minor change for making .f90 version.
c>> 1997-06-18 DRSNLAGB Krogh  Changes to improve C portability.
c>> 1994-11-02 DRSNLAGB Krogh  Changes to use M77CON
c>> 1994-09-14 DRSNLAGB CLL Set IV(OUTLEV) = 0 for comparing output.
...
c     Demo driver for SNLAGB. A variant of the nonlinear LS code NL2SOL.
c     SNLAGB handles bounded variables.  Requires values of the function
c     and the Jacobian matrix.
c     ------------------------------------------------------------------
c--S replaces "?": DR?NLAGB, ?NLAGB, ?CALCR, ?CALCJ, ?IVSET, ?KEY
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      print'(1x,a)',
     * 'Program DRSNLAGB.. Demo driver for SNLAGB.',
     * '   A variant of NL2SOL.',
     * '   SNLAGB requires values of the function and the Jacobian.',
     * '   SNLAGB handles bounds on variables.',
     * ' ',
     ...
     * ' '
      ...
      call SNLAGB(NDATA, NC, COEF, BND, SCALCR, SCALCJ, IV, LIV, LV, V)
