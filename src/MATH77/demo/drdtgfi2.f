c  File: DRDTGFI2.[F|FOR] CONTAINS DRDTGFI2 AND DTGFCN.
      program DRDTGFI2
c>> 2001-07-16 DRDTGFI2 Krogh Added exponent 0 to some constants.
c>> 2001-05-22 DRDTGFI2 Krogh Minor change for making .f90 version.
c>> 1997-07-01 DRDTGFI2 Krogh Reversed subscripts in B (CLL suggestion)
c>> 1997-06-20 DRDTGFI2 Krogh Minor changes to get proper C conversion.
c>> 1997-06-18 DRDTGFI2 CLL
c>> 1996-03-04 DRDTGFI2 CLL
c>> 1996-02-02 DRDTGFI2 CLL
c>> 1995-10-31 DRDTGFI2 CLL
c   Demo driver for DTGFI, DTGGRD, DTGPD, etc.
c   Tests search when extrapolating.
c     ------------------------------------------------------------------
c--D replaces "?": DR?TGFI2, ?TGFI, ?TGGRD, ?TGPD, ?TGFCN, ?TGPRG
c     ------------------------------------------------------------------
      ...
      double precision q(2), qtab(2,meval)
      ...
      logical  WANTDZ
      character title*75
        data qtab / 0.0d0,0.9d0,   -2.0d0,-1.1d0,
     *           -0.8d0,-0.7d0,  -2.7d0, 1.5d0,
     *            0.0d0,0.9d0,   -2.0d0,-0.5d0,
     *           -0.8d0,-0.7d0,  -2.0d0,0.7d0/
      data X /
     ...
     *-0.88827D0,-0.60000D0,-0.88620D0, 0.08600D0 /
      ...
      data wantdz / .true. /
c     ------------------------------------------------------------------
      write(*,'(a/a)') ' Program DRDTGFI2.  Demo driver for DTGFI,',
     *                 '                    DTGGRD, DTGPD, etc.'
      call DTGGRD(X,Y,NP,IP,W,TRIANG,MT,Bdry,MB,NT, INFO)
      call DTGPRG(X,Y,NP,TRIANG,Bdry,info(2),NT)
      if (INFO(1) .NE. 0) then
         write(*,'(a,i5)')
