      PROGRAM drddasl4
c>> 2008-10-29 DRDDASL4 Krogh Fixed print output for C.
c>> 2008-10-26 DRDDASL4 Krogh Changed Fortran 90 type statement to F77.
c>> 2008-10-26 DRDDASL4 Krogh Moved Format statements up for C conv.
c>> 2008-10-24 DRDDASL4 Krogh Removed in INCLUDE statement & cDEC$...
c>> 2008-09-17 DRDDASL4 Hanson added starting values for y'
c>> 2008-08-26 DRDDASL4 Hanson added row dimension to evaluator
c>> 2001-10-11 DRDDASL4  R. J. Hanson Example 4 Code, with Download
...
c--D replaces "?": DR?DASL4, ?DASLX, ?DASSF4, ?COPY, ?GEFA, ?GESL, ?DOT
...
c     This is flagged by INFO(5)=-6.
     ...
     & rwork(lrw),tout(nepoch),b0,b1,b2,b3,b4,ddot,cj
...
c++ Substitute for NDIG below
      ...
      DOUBLE PRECISION delta(nstate),gy(nstate,nstate),
...
c     Set initial conditions for state and variational equations:
      y(1)=0.d0
      yprime(1)=0.d0
      CALL dcopy (neq, y, 0, y, 1)
      CALL dcopy (neq, yprime, 0, yprime, 1)
      y(1)=b0
      y(nstate+1)=1.d0
...
c     Branch here to re-enter routine.
   50   CALL ddaslx (ddassf4, neq, t, y, yprime, tout(L), info, rtol,
...
c     Will need partials for IRES=1,2:
        IF (ires.le.2) THEN
          gy(1,1)=0.d0
          gp(1,1)=0.d0
          CALL dcopy (nstate**2, gy, 0, gy, 1)
          CALL dcopy (nstate*nvareq, gp, 0, gp, 1)
...
c     Extract DY/DP:
          CALL dcopy (nstate*nvareq, y(nstate+1), 1, yp, 1)
c     Extract (DY/DP)':
          CALL dcopy (nstate*nvareq, yprime(nstate+1), 1, ypp, 1)
        END IF
...
c     Update DG/DP = DG/DP - D (Y')/DP:
          DO 70 j=1,nvareq
            DO 60 i=1,nstate
              gp(i,j)=gp(i,j)+ddot(nstate,gy(i,1),nstate,yp(1,j),1)-
     &         ypp(i,j)
   60       CONTINUE
   70     CONTINUE
c     Move the results where the integrator uses them:
          CALL dcopy (nstate, delta, 1, rwork(iwork(4)), 1)
          CALL dcopy (nstate*nvareq, gp, 1, rwork(iwork(4)+nstate), 1)
          GO TO 50
...
c     This is where savings occur, compared to solving a full system.
          CALL dgefa (d, nstate, nstate, ipvt, ier)
...
c     Index to place solutions of corrector equations.            
          k=iwork(4)
          DO 90 j=1,nvareq+1
            CALL dgesl (d, nstate, nstate, ipvt, rwork(k), 0)
            k=k+nstate
   90     CONTINUE
...
c%% for(_n=4L; _n < sizeof(y)/sizeof(double); _n+=4)
...
c%%     y[_n], y[_n+1], y[_n+2], y[_n+3]);
  ...
  110 CONTINUE
      ...
      SUBROUTINE ddassf4 (t, y, yprime, delta, d, ldd, cj, ires, rwork,
     & iwork)
  
c     Dummy routine for reverse communication:
c     This is a double precision version.
      DOUBLE PRECISION t,y(*),yprime(*),delta(*),d(ldd,*),cj,rwork(*)
