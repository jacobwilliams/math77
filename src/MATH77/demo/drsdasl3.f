      PROGRAM drsdasl3
C>> 2009-10-28 DRSDASL3 Remove implicit none.
C>> 2009-10-21 DRSDASL3 Hanson/Krogh Fixed initialization.
C>> 2008-09-04 DRSDASL3 Hanson added starting computation of y'
C>> 2008-08-26 DRSDASL3 Hanson added row dimensions to evaluators
C>> 2001-10-11 DRSDASL3  R. J. Hanson Example 3 Code, with Download
...
C--S replaces "?": DR?DASL3, ?DASLS, ?DASLX, ?DASSF3, ?COPY, ?ROTG
C Version 6, Intel 10.1
      EXTERNAL sdassf3,r1mach,sdasls
...
C++ Substitute for NDIG below
      ...
      REAL             t,y(neq),yprime(neq),tout,rtol(neq),atol(neq),
...
c     Assign initial y, and guess for y', then get initial y'.
      do 25 i = 1, neq
        ...
        yprime(i) = 0.E0
 25   continue
c (Actual initial values for y(1) is set in sdassf3.)
...
C     Allow more than nominal number of steps.
      info(12)=50000
      length=1.1e0
C     Use partial derivatives provided in SDASSF3, and do our own
C     linear algebra.
      info(5)=8
      DO 30 i=1,10
C     Integrate from T=I-1 to TOUT=T+1.  Final TOUT=10.
        t=i-1
        tout=t+1
        CALL sdaslx (sdassf3, neq, t, y, yprime, tout, info, rtol, atol,
...
C     Conservation of length is not enforced.
      REAL             t,y(*),yprime(*),delta(*),d(ldd,5),cj,rwork(*),
     & dc(4),ds(4),u,v,lsq,mg,tj,yt(5)
      INTEGER ires,iwork(*),ldd
      LOGICAL factor
 
      REAL             one,two,zero,mass,length,gravity
      PARAMETER  (one=1.e0,two=2.e0,zero=0.e0,mass=1.e0,length=1.1e0,
     & gravity=9.806650e0)
...
C Count number of factorization steps.
          ...
          CALL srotg (d(1,1), d(3,1), dc(1), ds(1))
...
C Eliminate now in planes 2 and 5, column 2.
          CALL srotg (d(2,2), d(5,2), dc(2), ds(2))
...
C Eliminate in planes 3 and 5, column 3.
          CALL srotg (d(3,3), d(5,3), dc(3), ds(3))
...
C Eliminate in planes 4 and 5, column 4.
          CALL srotg (d(4,4), d(5,4), dc(4), ds(4))
