      PROGRAM DRDJACG2
c>> 2009-10-28 DRDJACG2 Krogh -- Remove implicit none.
c>> 2009-10-26 DRDJACG2 Krogh -- *'s used for dims for NAG compiler.
c>> 2008-11-01 DRDJACG2 Hanson -- Added row dim parameters to evaluators
c>> 2006-12-19 DRDJACG2 Hanson -- Added tot. energy constraint in 3rd ex
c>> 2006-04-11 DRDJACG1 Hanson -- Reduced lengths of djacg work arrays.
c>> 2006-04-09 DRDJACG2  Krogh Fixed dimension of YSCALE.
c>> 2003-07-08 DRDJACG2  R. J. Hanson Fix accum. option with djacg().
c>> 2002-02-01 DRDJACG2  R. J. Hanson Example 2 Code, with Download
...
C     is integrated (using DDASSFC) using these two constraints.
C     In the second integration the problem is transformed from
C     "Index 3" to "Index 0" by differentiating the length constraint
C     three times.  The routine DDASSFD uses these three constraints.
C     A total energy constraint is added in routine DDASSFE.
...
c--D replaces "?": DR?JACG2,?DASLX,?DASSFC,?DASSFD,?DASSFE,?COPY,?JACG

      EXTERNAL DDASSFC, DDASSFD, DDASSFE
...
c++ Substitute for NDIG below
      ...
      DOUBLE PRECISION T, Y(5), YPRIME(5), TOUT,
...
C     When the solution first drifts away from the constraints, stop.
        T=dble(I-1)
        TOUT=T+10.D0
        CALL DDASLX (DDASSFC, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL,
...
C     Integrate from T=I-1 to TOUT=T+1.
        T=dble(I-1)
        TOUT=T+10.D0
        CALL DDASLX (DDASSFD, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL,
     +    ATOL, IDID, RWORK, LRW, IWORK, LIW)
...
C     Integrate from T=I-1 to TOUT=T+1.
        T=dble(I-1)
        TOUT=T+10.D0
        CALL DDASLX (DDASSFE, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL,
     +    ATOL, IDID, RWORK, LRW, IWORK, LIW)
...
C  should be smaller than the tolerances used.
   ...
   80 CONTINUE
      ...
      SUBROUTINE DDASSFC(T,Y,YPRIME,DELTA,D,LDD,CJ,IRES,RWORK,IWORK)
C     Routine for the swinging simple pendulum problem,
C     with constraints on the index 2 and 3 equations.
      DOUBLE PRECISION T, Y(*), YPRIME(*), DELTA(*), D(LDD,*),
     +  CJ, RWORK(*), LSQ, MG
      INTEGER IRES, IWORK(*),LDD

      DOUBLE PRECISION ONE, TWO, ZERO, MASS, LENGTH, GRAVITY

C     Use numerical derivatives but based on calls to DJACG.
      INTEGER MODE, IOPT(5), IWK(21)
      DOUBLE PRECISION WK(3*5+18), F(5), FACC(5),
     .  FACJ(5), YSCALE(5)

      PARAMETER (ONE=1.D0, TWO=2.D0,
     +  ZERO=0.D0, MASS=1.D0, LENGTH=1.1D0, GRAVITY=9.806650D0)
...
C This is a Math a la Carte code for numerical derivatives.
        CALL DJACG (MODE, 5, 5, Y,
     .              F, D, LDD, YSCALE,
     .              FACJ, IOPT, WK, 33, IWK, 21)
        IF(MODE .gt. 0) GO TO 10
        IF(MODE .lt. 0) THEN
            PRINT
     1      '('' DDASSFC: Initial error in argument number '',I2)',-MODE
            STOP '1'
          END IF
...
C All constraint partials are computed.  Count an evaluation.
      ...
      SUBROUTINE DDASSFD(T,Y,YPRIME,DELTA,P,LDP,CJ,IRES,RWORK,IWORK)
C
C     Routine for the swinging simple pendulum problem,
C     with constraints on the index 1,2 amd 3 equations.
C     Use P(:,:) to distinguish from D(:,:) in routine DDASSFC.
      INTEGER IRES, IWORK(*), LDP
      DOUBLE PRECISION T, Y(*), YPRIME(*), DELTA(*), P(LDP,*),
     +  CJ, RWORK(*), LSQ, MG
C     Use numerical derivatives but based on calls to DJACG.
...
C This is the setup call.
      IF(IRES .EQ. 0) THEN
        ...
        Y(5)=ZERO
        YPRIME(1)=ZERO
        CALL DCOPY(5,YPRIME,0,YPRIME,1)
        yprime(4)=-gravity
        KR=0
        KF=0
        KC=0
...
C This is a Math a la Carte code for numerical derivatives.
     ...
     .              FACJ, IOPT, WK, 33, IWK, 21)
        ...
        IF(MODE .lt. 0) THEN
            PRINT
     1     '('' DDASSFD: Initial error in argument number '',I2)', -MODE
            STOP '1'
          END IF
C All system partials are computed.  Count an evaluation.
        KF=KF+1
...
C All constraint partials are computed.  Count an evaluation.
      ...
      SUBROUTINE DDASSFE(T,Y,YPRIME,DELTA,P,LDP,CJ,IRES,RWORK,IWORK)
C
C     Routine for the swinging simple pendulum problem,
C     with constraints on the index 1,2 amd 3 equations.
...
C     Use numerical derivatives but based on calls to DJACG.
      INTEGER MODE, IOPT(2), IWK(21)
      DOUBLE PRECISION WK(43), F(5), FACC(5),
     .  FACJ(5), YSCALE(1)

      DOUBLE PRECISION HALF, ONE, TWO, THREE, ZERO, MASS, LENGTH,
     +                 GRAVITY, LSQ, MG
...
C evaluation point.
        ...
        END IF
C This is a Math a la Carte code for numerical derivatives.
        CALL DJACG (MODE, 4, 5, Y,
     .              F, P(6,1), LDP, YSCALE,
     .              FACC, IOPT, WK, 43, IWK, 21)
        IF(MODE .gt. 0) GO TO 20
        IF(MODE .lt. 0) THEN
            PRINT
     1      '('' DDASSFE: Initial error in argument number '',I2)',-MODE
            STOP '2'
          END IF
C All constraint partials are computed.  Count an evaluation.
         KC=KC+1
