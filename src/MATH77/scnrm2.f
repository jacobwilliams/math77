...
c>> 1998-05-11 SCNRM2  Krogh   Minor changes for conversion to C.
c>> 1998-05-07 SCNRM2  Krogh   Adapted new algorithm from SNRM2
c>> ....
C>> 1985-10-02 SCNRM2 Lawson  Initial code.
C
C     EUCLIDEAN NORM OF THE COMPLEX N-VECTOR STORED IN X() WITH STORAGE
C     INCREMENT INCX .
...
C     ------------------------------------------------------------------
      ...
      real             SUM, SUMX, XA, XB
      real             ABIG,ASMALL,DBIG,DSMALL,FBIG,FSMALL,TBIG,TSMALL
...
c                      Loop when no big number yet encountered.
         do 100 I = 1, NN, INCX
            XA = abs(real(X(I)))
            XB = abs(aimag(X(I)))
            if (XA + XB .lt. TSMALL) then
               SUMX = SUMX + (FSMALL * XA)**2 + (FSMALL * XB)**2
            else
...
c                      Loop when we have at least one big number.
         do 300 I = I, NN, INCX
            XA = abs(real(X(I)))
            XB = abs(aimag(X(I)))
            if (XA + XB .gt. TSMALL) then
               if (XA + XB .gt. TBIG) then
                  SUMX = SUMX + (FBIG * XA)**2 + (FBIG * XB)**2
               else
                  SUM = SUM + XA**2 + XB**2
               end if
            end if
  300    continue
         if ((SUMX .le. 1.E10) .and. (SUM .ge. 1.E-10)) then
