...
c>> 2008-11-26 DPRTSV Krogh Changed FMT2 for Fortran standard change
c>> 2008-11-26 DPRTSV Krogh in how P and F interact in formats.
c>> 2001-06-08 DPRTSV Krogh  Increased length of FMT1 and FMT2.
c>> 2001-05-25 DPRTSV Krogh  Added a comma in a format.
c>> 2001-01-16 DPRTSV Krogh  Minor fix for fussy C compilers.
c>> 1996-06-27 DPRTSV Krogh  Changes to use .C. and C%%.
c>> 1996-01-23 DPRTSV Krogh  Got in code for C conversion.
...
C
         do 40 I=1,M
           if(STAR) then
             if(BLKNAM) then
               if (MODE .eq. 1) then
                 write (*,FMT2(1)) I,' ',(1.D4*A(I,J),J=J1,J2)
               else
                 write (*,FMT2(2)) I,' ',(A(I,J),J=J1,J2)
               end if
             else
               if (MODE .eq. 1) then
                 write (*,FMT2(1)) I,NAMES(I),(1.D4*A(I,J),J=J1,J2)
               else
             ...
             if(BLKNAM) then
               ...
               write (UNIT,FMT2(MODE)) I,NAMES(I),(1.D4*A(I,J),J=J1,J2)
             endif
           endif
 40      continue
   50 continue
