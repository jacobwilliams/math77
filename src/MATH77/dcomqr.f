...
c>> 2001-01-24 DCOMQR Krogh  ZSQRT -> ZSQRTX to avoid C lib. conflicts.
c>> 1996-04-27 DCOMQR Krogh  Changes to use .C. and C%%.
c>> 1996-03-30 DCOMQR Krogh  Added external statement.
C>> 1996-01-18 DCOMQR Krogh  Added M77CON statements for conv. to C.
...
c     ------------------------------------------------------------------
      ...
      DOUBLE PRECISION D1MACH,HI(NM,N),HR(NM,N),MACHEP
      DOUBLE PRECISION NORM,SI,SR,TI,TR,XI,XR,YI,YR,ZZI,ZZR
c++ Default NO_COMPLEX = .C. | (.N. == 'D')
c++ Default COMPLEX = ~NO_COMPLEX
c++ CODE for COMPLEX is inactive
c      COMPLEX Z(N), Z3
c++ CODE for NO_COMPLEX is active
C++ Replace "DZABS" = .N.//.Y.//"ABS"
      EXTERNAL DZABS
      DOUBLE PRECISION DZABS, Z(2*N), Z1(2), Z2(2)
      DOUBLE PRECISION ZDENOM(2),ZIN(2),ZNUM(2),ZOUT(2),ZQUOT(2)
      EXTERNAL ZQUO, ZSQRTX
c++ END
C
...
c++ CODE for COMPLEX is inactive
c         NORM = ABS(CMPLX(HR(I,I-1),HI(I,I-1)))
c++ CODE for NO_COMPLEX is active
         Z1(1) = HR(I,I-1)
         Z1(2) = HR(I,I-1)
         NORM = DZABS(Z1)
...
c++ CODE for COMPLEX is inactive
c      Z3 = SQRT(CMPLX(YR**2-YI**2+XR,2.0*YR*YI+XI))
c      ZZR = REAL(Z3)
c      ZZI = AIMAG(Z3)
c++ CODE for NO_COMPLEX is active
      ZIN(1) = YR**2 - YI**2 + XR
      ZIN(2) = 2.D0*YR*YI + XI
      CALL ZSQRTX(ZIN,ZOUT)
      ZZR = ZOUT(1)
      ZZI = ZOUT(2)
c++ END
  ...
  310 CONTINUE
c++ CODE for COMPLEX is inactive
c      Z3 = CMPLX(XR,XI) / CMPLX(YR+ZZR,YI+ZZI)
c      SR = SR - REAL(Z3)
c      SI = SI - AIMAG(Z3)
c++ CODE for NO_COMPLEX is active
      ZNUM(1) = XR
      ...
      ZDENOM(1) = YR + ZZR
      ZDENOM(2) = YI + ZZI
c                      ZQUO computes quotient of two complex numbers.
...
C     ********** A ROOT FOUND **********
  660 continue
c++ CODE for COMPLEX is inactive
C      Z(EN) = CMPLX(HR(EN,EN)+TR,HI(EN,EN)+TI)
c++ CODE for NO_COMPLEX is active
      Z(2*EN-1) = HR(EN,EN) + TR
      Z(2*EN) = HI(EN,EN) + TI
c++ END
      EN = ENM1
      GO TO 220
