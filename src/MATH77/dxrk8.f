      subroutine DXRK8(TS, Y, OPT, IDAT, DAT, WORK)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c Time-stamp: <2009-10-19 12:24:07 root>
c>> 2009-10-15 DXRK8 Krogh Latest version of new stepsize control.
c>> 2008-12-22 DXRK8 Krogh Many changes to put in new stepsize control.
c>> 2008-02-28 DXRK8 Krogh Lots of changes in usage, bug fixes.
c>> 2007-09-11 DXRK8 Krogh Fixed flag on max step output prior to final.
c>> 2007-09-11 DXRK8 Krogh Fixed DAT(MAXINC) getting too small.
c>> 2007-09-10 DXRK8 Krogh Some options were not processed.
c>> 2007-09-10 DXRK8 Krogh Stored 0 in IDAT at end Err. Tol. Info.
c>> 1997-12-18 DXRK8 Krogh Modifications for MATH77 library.
c>> 1997-02-24 DXRK8 Krogh Initial code.
c--D replaces "?": ?XRK8,?XRK8A,?XRK8F,?XRK8I,?XRK8N,?XRK8O,?XRK8X,?MESS
c
...
c from dxrk8f or from the users code as a result of using reverse
c communication.
c   From dxrk8f:  call dxrk8g(T, Y, F, IDAT)
c   Reverse Comm: call dxrk8g(TS, Y, WORK, IDAT)
c After a return from dxrk8, if one is continuing an integration, one
c should call dxrk8a instead of dxrk8 as follows
c                 call dxrk8a(TS, Y, F, IDAT, DAT, WORK)
...
c available in various arrays; and OPT(1) = 0.D0 to indicate no options
...
c   TS(2)  Current stepsize, h.  If h=0.D0 it is selected automatically.
...
ctol_i (see just above) is A2.  XRKRE1=3 (A3=e**(.75D0), B3=e**(.75D0))
...
ceffect as option 2 followed by option 1 with A1 increased by 1.D0 each
...
cset.  XRKDIG=8 (A8=0.D0) A8 = 0.D0, turns off diagnostic output for an
cequation group, =1.D0 turns on output giving information used in
ccomputing the error measure for the groups.  XRKIN=9 (A9=1.D0) A9 =
c1.D0 turns on interpolation, = 0.D0 turns it off.  If one is doing
cinterpolation to an output point, no valid results are returned for
cequations which are in groups with A9 = 0.D0.  If you have G-Stops,
cmake sure that all the y's used in computing them are interpolated for.
cXRKXP=10 (A10=0.D0) A10 = 1.D0 causes extra precision to be used in
caccumulating Y, =0.D0 uses standard floating point precision.  (This
...
c XRKEC=27 (A27=4.6D0, B27=.16D0)  Logic in the code attempts to select
...
c    not be obtained.
c
c XRKIFS=28 (A28=.1D0) Fraction of a step by which an interpolation
...
c        when DXRK8 (or DXRK8A) is called, and when it makes a return to
...
c   IDAT(1) = 0  Starting an integration, not set on return from DXRK8.
...
c              = 22 Bad value for IDAT(1) on entry to DXRK8A.
...
c        to the program calling DXRK8 with IDAT(1) = 3, and IDAT(2) =
...
c        referenced by DXRK8.  If one provides a negative number here,
c        its absolute value is used, and the value in IDAT(5) is
c        replaced by the space in IDAT actually required by DXRK8.
...
c LEPS   contains EPS = smallest number such that 1.D0 + EPS .ne. 1.D0.
...
c   rejected.  If there is no error control, DAT(LERTRY) = 0.D0
...
c NEGPOS -1.D0 if the stepsize if < 0, and 1.D0 otherwise.
...
c EN     Passed to DXRK8N.  EN(1) is the square of the fifth order error
...
c  3    23   (A3=e**(.75D0), B3=e**(.75D0)) Relative error
...
c 28 10*(LINTX+100)+2   (A28=.1D0) Fraction of step by which
...
c Formal arguments
      integer IDAT(*)
      double precision TS(*), Y(*), OPT(*), DAT(*), WORK(*)
c Parameters used for options
      double precision XRKEND, XRKEQG, XRKAE, XRKRE1, XRKRE2, XRKAEV,
     ...
     3  XRKXG, XRKIG,XRKEC, XRKIFS
      parameter (XRKEND=0.D0, XRKEQG=1.D0, XRKAE=2.D0, XRKRE1=3.D0,
     1  XRKRE2=4.D0, XRKAEV=5.D0, XRKREV=6.D0, XRKNOE=7.D0,
     2  XRKDIG=8.D0, XRKIN=9.D0, XRKXP=10.D0, XRKFR1=12.D0,
     3  XRKFR2=13.D0, XRKOR=14.D0, XRKMAX=15.D0, XRKMIN=16.D0,
     4  XRKEPS=17.D0, XRKMXS=20.D0, XRKEOS=21.D0, XRKTI=22.D0,
     5  XRKTS=23.D0, XRKTDT=24.D0, XRKXG=25.D0, XRKIG=26.D0,
     6  XRKEC=27.D0, XRKIFS=28.D0)
...
c Local variables
     ...
     2  LIDAT, LLERR, LO, LWORK, MAXLER, MINLER, N, NEQ, NEQNOE, NEXT
      external D1MACH
      double precision D1MACH, ERR1, ERR2, DIFFTS
...
cAG Options must be turned on with 1.D0 and off with 0.D0, you $C
c   used $F.$E
...
cAM IDAT(4:7):$B
     ...
     * LTXTAK=448,LTXTAL=  1,LTXTAM= 59)
      ...
      data MTXTAA/'DXRK8$BUndefined option.$EEquation indexes for groups
     ...
     *EOptions must be turned on with 1.D0 and off with 0.D0, you used $
...
c                      Data to store in DAT.
          ...
          if (L .eq. 3) then
            DAT(KAT - 99) = OPT(LO+2)
            if (DAT(KAT-100) .lt. 4.D0 * DAT(KAT-99)) then
              IERR = 10
              go to 700
            end if
          end if
...
c             Initialize some stuff that depends on user input.
 ...
 540    continue
      ...
      if (NEQ .ne. NEQNOE) then
        DAT(LERTRY) = 100.D0 / dble(NEQ - NEQNOE)
      end if
      DAT(LERTRY+1) = 6.D0
...
c **************************** Integrate *******************************
c
 600  call DXRK8A(TS, Y, OPT, IDAT, DAT, WORK)
      return
c
c *********************** Error Processing *****************************
c
c   Entry when insufficient space in DAT
 ...
 700  MACT(3) = IERROR(IERR)
      MACT(4) = LERMSG(IERR)
      call DMESS(MACT, MTXTAA, IDAT, WORK)
      MACTV(3) = LO + L
      call DMESS(MACTV, MTXTAB, IDAT(4), OPT)
      IDAT(1) = 4
      IDAT(2) = MACT(3)
      return
      end

      subroutine DXRK8A(TS, Y, F, IDAT, DAT, WORK)
...
c Formal arguments
      integer IDAT(*)
      double precision TS(3), Y(*), F(*), DAT(*), WORK(*)
c
c Parameters used to reference DAT
      integer HA, LASTE3, LEPS, LERR, HMAX, LERTRY, LAH,
     1  HCAU, LHMAXU, LHMIN, LINTX, LOCTGS, LOCSAV, LOCTY, LOCTXG,
...
c Parameters used in exponential averaging for the stepsize adjustment.
      double precision P, P5PINV, PINV, W
      parameter (P=8.D0, PINV=1.D0/P, P5PINV=.5D0*PINV, W=.1D0)
      double precision WI1A, WI1B, WI2A, WI2B
      parameter (WI1A=W/(1.D0-W)**2)
      parameter (WI1B=(1.D0-2.D0*W)/(1.D0-W)**2)
      parameter (WI2A=(2.D0*W)/(1.D0-W)**3)
      parameter (WI2B=(1.D0-3.D0*W)/(1.D0-W)**3)
c      parameter (WI3A=(3.D0*W)/(1.D0-W)**4)
c      parameter (WI3B=(1.D0-4.D0*W)/(1.D0-W)**4)
c      double precision  WQ1A,WQ2A,WQ3A,WQ1B,WQ2B,WQ3B,WQ1C,WQ2C,WQ3C
c      parameter(WQ1A=(1.D0-W)*(W**2+W+1.D0)/W**2)
c      parameter(WQ2A=(1.D0-W)*(W**2+W-2.D0)/W**2)
c      parameter(WQ3A=(1.D0-W)**3/W**2)
c      parameter(WQ1B=(1.D0-W)**2*(-W-2.D0)/W**2)
c      parameter(WQ2B=(1.D0-W)**2*(-W**2-3.D0*W+4.D0)/W**2)
c      parameter(WQ3B=-2.D0 * (1.D0-W)**4/W**2)
c      parameter(WQ1C=((1.D0-W)**3)/W**2)
c      parameter(WQ2C=(-2.D0*(1.D0-W)**4)/W**2)
c      parameter(WQ3C=((1.D0-W)**5)/W**2)

      double precision WL1A, WL2A
      parameter (WL1A=(1.D0-W**2)/W, WL2A=-(1.D0-W)**2/W)
c      parameter (WL1B=-(1.D0-W)**2/W, WL2B=(1.D0-W)**3/W)

c      double precision AQ, BQ, CQ, AL, BL
...
c Local Variables
      ...
      double precision EN(2), ERR, H, TP, TP1, TP2
      external DXRK8X
      double precision DXRK8X
c
c Integration coefficients
      double precision C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C14,
     ...
     E  D715, D716
      parameter ( C2  = 0.526001519587677318785587544488D-01,
     1  C3  = 0.789002279381515978178381316732D-01,
     2  C4  = 0.118350341907227396726757197510D+00,
     3  C5  = 0.281649658092772603273242802490D+00,
     4  C6  = 0.333333333333333333333333333333D+00,
     5  C7  = 0.25D+00,
     6  C8  = 0.307692307692307692307692307692D+00,
     7  C9  = 0.651282051282051282051282051282D+00,
     8  C10 = 0.6D+00,
     9  C11 = 0.857142857142857142857142857142D+00,
     A  C14 = 0.1D+00,
     B  C15 = 0.2D+00,
     C  C16 = 0.777777777777777777777777777778D+00 )
      parameter ( B1 =   5.42937341165687622380535766363D-2,
     1  B6 =   4.45031289275240888144113950566D0,
     2  B7 =   1.89151789931450038304281599044D0,
     3  B8 =  -5.8012039600105847814672114227D0,
     4  B9 =   3.1116436695781989440891606237D-1,
     5  B10 = -1.52160949662516078556178806805D-1,
     6  B11 =  2.01365400804030348374776537501D-1,
     7  B12 =  4.47106157277725905176885569043D-2 )
      parameter ( BHH1 = 0.244094488188976377952755905512D+00,
     1  BHH2 = 0.733846688281611857341361741547D+00,
     2  BHH3 = 0.220588235294117647058823529412D-01 )
      parameter ( ER1 =  0.1312004499419488073250102996D-01,
     1  ER6  = -0.1225156446376204440720569753D+01,
     2  ER7  = -0.4957589496572501915214079952D+00,
     3  ER8  =  0.1664377182454986536961530415D+01,
     4  ER9  = -0.3503288487499736816886487290D+00,
     5  ER10 =  0.3341791187130174790297318841D+00,
     6  ER11 =  0.8192320648511571246570742613D-01,
     7  ER12 = -0.2235530786388629525884427845D-01 )
      parameter ( A21 =    5.26001519587677318785587544488D-2,
     1  A31 =    1.97250569845378994544595329183D-2,
     2  A32 =    5.91751709536136983633785987549D-2,
     3  A41 =    2.95875854768068491816892993775D-2,
     4  A43 =    8.87627564304205475450678981324D-2,
     5  A51 =    2.41365134159266685502369798665D-1,
     6  A53 =   -8.84549479328286085344864962717D-1,
     7  A54 =    9.24834003261792003115737966543D-1,
     8  A61 =    3.7037037037037037037037037037D-2,
     9  A64 =    1.70828608729473871279604482173D-1,
     A  A65 =    1.25467687566822425016691814123D-1,
     B  A71 =    3.7109375D-2,
     C  A74 =    1.70252211019544039314978060272D-1,
     D  A75 =    6.02165389804559606850219397283D-2,
     E  A76 =   -1.7578125D-2 )
      parameter ( A81 =    3.70920001185047927108779319836D-2,
     1  A84 =    1.70383925712239993810214054705D-1,
     2  A85 =    1.07262030446373284651809199168D-1,
     3  A86 =   -1.53194377486244017527936158236D-2,
     4  A87 =    8.27378916381402288758473766002D-3,
     5  A91 =    6.24110958716075717114429577812D-1,
     6  A94 =   -3.36089262944694129406857109825D0,
     7  A95 =   -8.68219346841726006818189891453D-1,
     8  A96 =    2.75920996994467083049415600797D1,
     9  A97 =    2.01540675504778934086186788979D1,
     A  A98 =   -4.34898841810699588477366255144D1,
     B  A101 =   4.77662536438264365890433908527D-1,
     C  A104 =  -2.48811461997166764192642586468D0,
     D  A105 =  -5.90290826836842996371446475743D-1,
     E  A106 =   2.12300514481811942347288949897D1,
     F  A107 =   1.52792336328824235832596922938D1,
     I  A108 =  -3.32882109689848629194453265587D1,
     J  A109 =  -2.03312017085086261358222928593D-2 )
      parameter ( A111 =  -9.3714243008598732571704021658D-1,
     1  A114 =   5.18637242884406370830023853209D0,
     2  A115 =   1.09143734899672957818500254654D0,
     3  A116 =  -8.14978701074692612513997267357D0,
     4  A117 =  -1.85200656599969598641566180701D1,
     5  A118 =   2.27394870993505042818970056734D1,
     6  A119 =   2.49360555267965238987089396762D0,
     7  A1110 = -3.0467644718982195003823669022D0,
     8  A121 =   2.27331014751653820792359768449D0,
     9  A124 =  -1.05344954667372501984066689879D1,
     A  A125 =  -2.00087205822486249909675718444D0,
     B  A126 =  -1.79589318631187989172765950534D1,
     C  A127 =   2.79488845294199600508499808837D1,
     D  A128 =  -2.85899827713502369474065508674D0,
     E  A129 =  -8.87285693353062954433549289258D0,
     F  A1210 =  1.23605671757943030647266201528D1,
     G  A1211 =  6.43392746015763530355970484046D-1 )
      parameter ( A141 =  5.61675022830479523392909219681D-2,
     1  A147 =  2.53500210216624811088794765333D-1,
     2  A148 = -2.46239037470802489917441475441D-1,
     3  A149 = -1.24191423263816360469010140626D-1,
     4  A1410 =  1.5329179827876569731206322685D-1,
     5  A1411 =  8.20105229563468988491666602057D-3,
     6  A1412 =  7.56789766054569976138603589584D-3,
     7  A1413 = -8.298D-3 )
      parameter ( A151 =  3.18346481635021405060768473261D-2,
     1  A156 =  2.83009096723667755288322961402D-2,
     2  A157 =  5.35419883074385676223797384372D-2,
     3  A158 = -5.49237485713909884646569340306D-2,
     4  A1511 = -1.08347328697249322858509316994D-4,
     5  A1512 =  3.82571090835658412954920192323D-4,
     6  A1513 = -3.40465008687404560802977114492D-4,
     7  A1514 =  1.41312443674632500278074618366D-1,
     8  A161 = -4.28896301583791923408573538692D-1,
     9  A166 = -4.69762141536116384314449447206D0,
     A  A167 =  7.68342119606259904184240953878D0,
     B  A168 =  4.06898981839711007970213554331D0,
     C  A169 =  3.56727187455281109270669543021D-1,
     D  A1613 = -1.39902416515901462129418009734D-3,
     E  A1614 =  2.9475147891527723389556272149D0,
     F  A1615 = -9.15095847217987001081870187138D0 )
      parameter ( D41  = -0.84289382761090128651353491142D+01,
     1  D46  =  0.56671495351937776962531783590D+00,
     2  D47  = -0.30689499459498916912797304727D+01,
     3  D48  =  0.23846676565120698287728149680D+01,
     4  D49  =  0.21170345824450282767155149946D+01,
     5  D410 = -0.87139158377797299206789907490D+00,
     6  D411 =  0.22404374302607882758541771650D+01,
     7  D412 =  0.63157877876946881815570249290D+00,
     8  D413 = -0.88990336451333310820698117400D-01,
     9  D414 =  0.18148505520854727256656404962D+02,
     A  D415 = -0.91946323924783554000451984436D+01,
     B  D416 = -0.44360363875948939664310572000D+01)
      parameter ( D51  =  0.10427508642579134603413151009D+02,
     1  D56  =  0.24228349177525818288430175319D+03,
     2  D57  =  0.16520045171727028198505394887D+03,
     3  D58  = -0.37454675472269020279518312152D+03,
     4  D59  = -0.22113666853125306036270938578D+02,
     5  D510 =  0.77334326684722638389603898808D+01,
     6  D511 = -0.30674084731089398182061213626D+02,
     7  D512 = -0.93321305264302278729567221706D+01,
     8  D513 =  0.15697238121770843886131091075D+02,
     9  D514 = -0.31139403219565177677282850411D+02,
     A  D515 = -0.93529243588444783865713862664D+01,
     B  D516 =  0.35816841486394083752465898540D+02)
      parameter ( D61 =  0.19985053242002433820987653617D+02,
     1  D66 = -0.38703730874935176555105901742D+03,
     2  D67 = -0.18917813819516756882830838328D+03,
     3  D68 =  0.52780815920542364900561016686D+03,
     4  D69 = -0.11573902539959630126141871134D+02,
     5  D610 =  0.68812326946963000169666922661D+01,
     6  D611 = -0.10006050966910838403183860980D+01,
     7  D612 =  0.77771377980534432092869265740D+00,
     8  D613 = -0.27782057523535084065932004339D+01,
     9  D614 = -0.60196695231264120758267380846D+02,
     A  D615 =  0.84320405506677161018159903784D+02,
     B  D616 =  0.11992291136182789328035130030D+02 )
      parameter ( D71  = -0.25693933462703749003312586129D+02,
     1  D76  = -0.15418974869023643374053993627D+03,
     2  D77  = -0.23152937917604549567536039109D+03,
     3  D78  =  0.35763911791061412378285349910D+03,
     4  D79  =  0.93405324183624310003907691704D+02,
     5  D710 = -0.37458323136451633156875139351D+02,
     6  D711 =  0.10409964950896230045147246184D+03,
     7  D712 =  0.29840293426660503123344363579D+02,
     8  D713 = -0.43533456590011143754432175058D+02,
     9  D714 =  0.96324553959188282948394950600D+02,
     A  D715 = -0.39177261675615439165231486172D+02,
     B  D716 = -0.14972683625798562581422125276D+03)
c
c Parameters for error messages
...
c[Last 2 letters of Param. name]  [Text generating message.]
cAA DXRK8$B
cAB IDAT(1) = $I is an error on  entry to DXRK8A.$E
...
cAP Raw($I:)/H**2 =$B
     ...
     * LTXTAP=  1)
      ...
      data MTXTAA/'DXRK8$BIDAT(1) = $I is an error on  entry to DXRK8A.$
     ...
     *nt computation of its value.$E '/
      data MTXTAB/'DXRK8$BError tolerance increased from $(E10.3) times$
...
C      double precision ESUMSQ, EMAXER
...
c Computing derivatives -- Starting location depends on which deriv.
 ...
 220  continue
      ...
      call DXRK8F(TS(1), Y, WORK(IDAT(2)), IDAT)
c
...
c (Most of the Y arguments do no contribute to the desired results.)
      call DXRK8N(IDAT, DAT, WORK, Y, Y, Y, DAT(LERR))
...
c Get the error scaled norm squared of the differences in F, Y (/H**2)
      call DXRK8N(IDAT, DAT, WORK(NEQ+1), DAT(LOCBF+1), DAT(LOCTY+1),
     1  DAT(LOCTY+1),  EN)
      if (EN(1) .eq. 0.D0) then
c                        No change in f, try increasing stepsize by 100.
        DAT(HA) = 100.D0 * DAT(HA)
      else
...
c          5 picked as it works reasonably well
      TP2 = 5.D0 * TP2 * (EN(1) * DAT(HA))**(-.125D0)
c                Reduce if concern for stiffness
      if (EN(2) .ne. 0.D0) TP2 = min(TP2, DAT(HA)* sqrt(EN(2)/EN(1)))
...
c G sign change, Try a small step to almost reach the sign change.
 2850 TP = abs(DAT(NEXTH))
      H = TS(2)
      if (abs(TP - H) .lt. DAT(LEPS) * 10.D0 * abs(TP)) then
...
c                Skip calculation of error estimate if no error control.
 3260 if (DAT(LERTRY) .eq. 0.D0) go to 3400
      call DXRK8N(IDAT, DAT, WORK(3*NEQ+1), WORK(9*NEQ+1),
     1  Y, DAT(LOCTY+1),  EN)

c Convert order 10 estimator, ERR, to an order 14 estimator, new ERR.

      EN(2) = 1.D-2 * (EN(2) + 1.D-3 * DAT(LASTE3))
...
c++  End
      ...
      if (IDAT(NERRSV) .ne. 0) then
        ...
        IDAT(NERRSV) = 1
        TP1 = .25D0 * DAT(LPHI) + .75D0 * DAT(PHI)
      end if
      DAT(HA) = exp(-PINV * TP1)
 ...
 3280 continue
      if (IDAT(LOCEPR) .ne. 0) then
        ...
        call DMESS(MACT1, MTXTAC, IDAT, DATERR)
      end if
...
c      AQ = max(AQ, AL)
      ...
      if (EN(1) .gt. EN(2)) then
        TP = DAT(LAH) + .75D0 * log(1.D-2*EN(1)/EN(2))
        if (TP .gt. AH) then
          ...
          if (IDAT(LOCEPR) .ne. 0) then
            ...
            call DMESS(MACT4, MTXTAD, IDAT(KSTIFF), DATERR)
          end if
          if ((TP .gt. DAT(LAH)) .and. (ERR .lt. 1.D-4)) TP = DAT(LAH)
          AH = TP
...
c                Print info. about error estimation
        call DMESS(MACT1, MTXTAE, IDAT, DATERR)
        K = IDAT(LOCEPR)
 ...
 3420     I1 = IDAT(K)
          if (I1 .ne. 0) then
            MACT2(3) = I1
            MACT2(5) = -IDAT(K+1)
            call DMESS(MACT2, MTXTAF, IDAT(K), WORK(3*NEQ+1))
            K = K + 2
            go to 3420
          end if
        end if
...
c             Accumulate t in extra precision
        ...
        DAT(J) = DXRK8X(TS(1), DAT(LOCTY), H, DAT(J))
        DAT(NEXTT) = TS(1)
        I1 = IDAT(K+1)
...
c                    First get the correction to Y.
          ...
          DAT(J) = DXRK8X(Y(I), DAT(LOCTY+I), TP, DAT(J))
          WORK(I+2*NEQ) = Y(I)
 3570   continue
        I1 = IDAT(K+1)
...
c **************** Get stepsize for starting the integration ***********
 ...
 3600 if (TS(2) .ne. 0.D0) then
        if (DAT(NEGPOS) * TS(2) .ge. 0.D0) go to 3650
        DATERR(1) = TS(2)
        IERR = 6
        go to 5000
      end if
...
c evaluations.
c
      if (DAT(LERR) .ne. 0.D0) then
c Set TP = est. (||y||_e / ||f||_e)^2
        TP = max(sqrt(DAT(LERR+1) / DAT(LERR)), 1.D-2)
        DAT(HA)=.05D0 * TP * (DAT(LERR+1)+DAT(LERR))**(-.0625D0)
      else
        DAT(HA) = .05D0
        TP = DAT(LERR+1) ** .0625D0
      end if
...
c                            Processed some kind of T output point
        ...
        if (DAT(LOCSAV) .eq. DAT(J)) then
          if (IDAT(1) .eq. 24) then
            if (DAT(NEGPOS) * DAT(J+1) .gt. 0.D0) then
              DAT(J) = DAT(J) + DAT(J+1)
              go to 3750
            end if
          end if
...
c  Store a value well beyond TS(3) so this output is ignored later.
            DAT(J) = sign((2.D0 * abs(TS(3)) + 1.D35), DAT(NEGPOS))
          end if
        end if
...
c
 ...
 3780 if (IDAT(J) .ne. 30) then
        I1 = IDAT(J+1)
        if (DAT(NEGPOS) * (DAT(I1) - DAT(LTOUT)) .lt. 0.D0) then
...
c (Needed if processing after got a G-Stop on the step already.)
          if (TS(1) .ne. DAT(NEXTT)) then
            if (DAT(NEGPOS) * (DAT(NEXTT) - TS(1)) .lt. 0.D0) then
c                        We extrapolated for a G-Stop
              TS(1) = DAT(LOCTXG)
              if (DAT(NEGPOS)*(TS(1)-TS(3)) .gt. 0.D0) TS(1) = TS(3)
              IDAT(NGHI) = -IDAT(NGHI)
              IDAT(NGLO) = IDAT(NGHI)
              go to 4200
            end if
...
c                  Check for output point
      if (DAT(NEGPOS) * (DAT(NEXTT) - DAT(LTOUT)) .ge. 0.D0) then
...
c                         Output to user.
 4250 if (IDAT(LREVO) .ne. 0) return
      call DXRK8O(TS, Y, IDAT, DAT)
      go to 400
c                         Call DXRK8F for a G-Stop
 4300 IDAT(2) = 0
      IDAT(3) = 0
 4310 IDAT(LWHYF) = 1
      if (IDAT(LREVF) .ne. 0) return
      call DXRK8F(TS(1), Y, WORK, IDAT)
 4320 continue
...
c  If G-Stop precedes next output, then output the G-Stop
      if (DAT(NEGPOS) * (TS(1) - DAT(LTOUT)) .le. 0.D0)  go to 4220
...
c               Some kind of error.
 5000 MACT(2) = ISTOP(IERR)
      MACT(3) = IERROR(IERR)
      MACT(4) = LERMSG(IERR)
      call DMESS(MACT, MTXTAA, IDAT, DATERR)
...
c   $
cAC DXRK8$B
cAD A bug in dxrk8, dxrk8i called when not allowed at T=$F.$E
      ...
      data MTXTAA/'DXRK8$BInterpolating outside of safe interval by $F o
     *f the stepsize at T=$F.$E'/
      data MTXTAB/'DXRK8$BA bug in dxrk8, dxrk8i called when not allowed
     * at T=$F.$E'/
c **** End of automatically generated text
c
...
c Check if ready.
      ...
      if (NIY .le. 0) then
        ...
        MACT(3) = 29
        DATERR(1) = T
        call DMESS(MACT, MTXTAB, IDAT, DATERR)
        stop
      end if

      J = IDAT(LOCINT)
      S = (T - WORK(J-2)) / WORK(J-1)
      if ((S .gt. WORK(1) + 1.D0) .or. (S .lt. -WORK(1))) then
c              Error interpolation outside allowed interval.
 ...
 80   continue
      ...
      subroutine DXRK8N(IDAT, DAT, V1, V2, Y, YOLD, EN)
c Computes squared norms of V1, V2, Y and F as well as Y^T F scaled by
c error tolerances.
c
c Formal arguments
      integer IDAT(*)
      double precision DAT(*), V1(*), V2(*), Y(*), YOLD(*), EN(2)
c
c Parameters used to reference DAT
      integer HA, LASTE3, LEPS, LERR, HMAX, LERTRY, LAH,
     1  HCAU, LHMAXU, LHMIN, LINTX, LOCTGS, LOCSAV, LOCTY, LOCTXG,
...
c Local variables
      ...
      double precision SV1, SV2, PV1, PV2, SY, TP, TP1, TP2
      J = LOCERR
      I2 = 0
      SV1 = 8.D-32
      SV2 = 1.D-32
...
c Separate for each Y -- mixed absolute / relative error test.
        ...
        TP2 = DAT(L+1) ** 2
        do 50  I = I1, I2
          TP = 1.D0 / (TP1 + TP2 * (Y(I)**2 + YOLD(I)**2))
          SV1 = SV1 + TP * V1(I)**2
          SV2 = SV2 + TP * V2(I)**2
 50     continue
      else if (K .eq. 5) then
...
c Vector mixed absolute / relative error test
        do 70 I = I1, I2
          TP = 1.D0/(DAT(L+2*I)**2+DAT(L+2*I+1)**2*(Y(I)**2+YOLD(I)**2))
          SV1 = SV1 + TP * V1(I)**2
          SV1 = SV1 + TP * V2(I)**2
 70     continue
      end if
      go to 20
c               End of subroutine DXRK8N
      end


      double precision function DXRK8X(VNEW, VOLD, VDIFF, VLOW)
...
c       stores intermediate results to memory.
      double precision VNEW, VOLD, VDIFF, VLOW
c  With perfect arithmetic this would return 0, if because of round-off
c  VNEW is a little too small, then DXRK8X > 0 which will give a little
c  boost to the Y on the next step.
      DXRK8X = (VOLD - VNEW) + VDIFF + VLOW
      return
      end
