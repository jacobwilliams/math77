      subroutine SUSETN ( Nin, M1in, M2in)
c  Base name of file: SUCOMP
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 2001-06-18 SUCOMP Krogh  Replaced "M1 .eq.. 0" with "M1 .eq. 0"
C>> 1996-04-30 SUCOMP Krogh  Removed redundant save statement.
C>> 1994-11-02 SUCOMP Krogh  Changes to use M77CON
C>> 1994-08-04 SUCOMP CLL New subrs: [D|S]USETN & [D|S]UGETN.
C>> 1993-11-11 CLL Changed Entries to separate Subroutines.
C>> 1990-01-23 CLL Replace M with MPWR in call to IERM1
C>> 1987-12-07 SUCOMP Lawson  Initial code.
c
c  The file SUCOMP contains a set of program units to perform
...
c        Subroutines
c
c    subroutine SUSETN ( N, M1, M2)
c    subroutine SUGETN ( N, M1, M2, L1, L2)
c    subroutine SUSET  ( VAL, KEY, Y9)
c
c    subroutine SUPRO  ( U,V,Y)
c    subroutine SUQUO  ( U1,V1,Y1)
c    subroutine SUSUM  ( U3,V3,Y3)
c    subroutine SUDIF  ( U4,V4,Y4)
c    subroutine SUSUM1 ( C5,V5,Y5)
c    subroutine SUDIF1 ( C6,V6,Y6)
c    subroutine SUPRO1 ( C7,V7,Y7)
c    subroutine SUQUO1 ( C8,V8,Y8)
c
c    subroutine SUSQRT (U,Y)
c    subroutine SUEXP  (U1,Y1)
c    subroutine SULOG  (U2,Y2)
c    subroutine SUPWRI (MPWR,U3,Y3)
c
c    subroutine SUSIN  (U,Y)
c    subroutine SUCOS  (U ,Y )
c    subroutine SUSINH (U ,Y )
c    subroutine SUCOSH (U ,Y )
c    subroutine SUATAN (U1,Y1)
c    subroutine SUATN2 (V2,U2,Y2)
c    subroutine SUASIN (U, Y)
c    subroutine SUACOS (U, Y)
c    subroutine SUTAN  (U, Y)
c    subroutine SUTANH (U, Y)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c--S replaces "?": ?UCOMP, ?UACOS, ?UASIN, ?UATAN, ?UATN2, ?UCOS
...
c     ==================================================================
c     subroutine SUSETN ( Nin, M1in, M2in)
c
...
c     ==================================================================
      subroutine SUGETN ( Nout, M1out, M2out, L1out, L2out)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
C
      if(V1(1) .eq. ZERO) THEN
         call ERMSG('SUQUO',5,0,'Denominator is zero.','.')
         return
      end if
      if(M1-1)80,90,100
C                                      VALUE
...
C
      if(V8(1) .eq. ZERO) then
         call ERMSG('SUQUO1',1,0,'Denominator is zero.','.')
         return
      endif
      if(M1-1)204,208,214
C                                      VALUE
...
C     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      subroutine SUATAN (U1,Y1)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      integer i, j, k
      integer N, M1, M2
      common/UCOM1/N,M1,M2
      save /UCOM1/
      real              D1, FAC, FAC2, U1(*),Y1(*)
      real                   ONE
      parameter(ONE = 1.0E0)
C
C     COMPUTE..                        Y= ATAN(U)
...
C
C
      subroutine SUATN2(V2,U2,Y2)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      integer i, j, k
      integer N, M1, M2
      common/UCOM1/N,M1,M2
      save /UCOM1/
      real              FAC1, FAC2, R, U2(*),V2(*),Y2(*)
      real                   ONE
      parameter(ONE = 1.0E0)
C
...
C     COMPUTE..                        Y= ASIN(U)
C
      call SUACS (.false., U1,Y1)
      return
      end
C     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      subroutine SUACOS (U1,Y1)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      integer N, M1, M2
      common/UCOM1/N,M1,M2
      save /UCOM1/
...
C     COMPUTE..                        Y= ACOS(U)
C
      call SUACS (.true., U1,Y1)
      return
      end
c     ==================================================================
      subroutine SUACS (ACOSIN, U1,Y1)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      logical ACOSIN
      integer i, j, k
      integer N, M1, M2
...
c                                     Error condition.
         if(ACOSIN) then
            call ERMSG('SUACOS',1,0,
     *      'Deriv of ACOS(x) is infinite at x = -1 or +1','.')
         else
            call ERMSG('SUASIN',1,0,
