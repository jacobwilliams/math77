...
c>> 2006-05-03 SILUPMD Krogh Started this debug routine
c
c--S replaces "?": ?ILUPMD, ?ILUPM, ?MESS
c
c Provide debugging information for Multidimensional Polynomial
c Interpolation with Look Up, silupm.
c Design/Code by Fred T. Krogh, Math a la Carte, Inc.
c
c *****************     Formal Arguments     ***************************
c
c These are exactly as for silupm.  See silupm.f for details.
c
c *****************     Variables Referenced     ***********************
c
c EOPT   Formal argument, see silupm.f.
c FDAT   Place to store floating point for error messages.
c I      Temporary index.
c IOPT   Formal argument, see silupm.f.
...
c        of the message.
c LUP    Formal argument, see silupm.f.
...
c METDIG Parameter for SMESS to set temporily the digits printed.
c NDEG   Formal argument, see silupm.f.
c NDIM   Formal argument, see silupm.f.
c NDIMI  Internal value for NDIM = number of dimensions.
c NTAB   Formal argument, see silupm.f.
...
c X      Formal argument, see silupm.f.
c XT     Formal argument, see silupm.f.
c Y      Formal argument, see silupm.f.
c YT     Formal argument, see silupm.f.
c
...
c
c       MAXDEG and MAXDIM Should agree with values silupm.f
...
cAA Inputs to SILUPM, with NDIM = $I, EOPT dim. of $I.$E
...
c   $
     ...
     * LTXTAZ=  1,LTXTBA=  1,LTXTBB=  1,LTXTBC=  1)
      ...
      data MTXTAA/'Inputs to SILUPM, with NDIM = $I, EOPT dim. of $I.$EI
...
c
      ...
      INTCHK(2) = IOPT(2)
      MACT(2) = MLOC(1)
      call SMESS(MACTDG, MTXTAA, INTCHK, FDAT)
 10   continue
      call SMESS(MACT, MTXTAA, INTCHK, FDAT)
      if (I .eq. 0) then
        if ((NDIMI .gt. MAXDIM) .or. (NDIMI .le. 0)) stop
     1    'Code requires 0 < NDIM < 11.'
        if (NTAB(NTABXT) .le. 0) then
c This block of code copied indented, else unchanged from silupm.f,
...
c### This block of code added to that from silupm
              MACTIV1(2) = I
              MACTIV1(5) = LT - J + 1
...
c       Get pointer to start of XT data for next dimension
 ...
 50     continue
c End of code included from SILUPM
        I = 2
...
c             1   2    3    4    5    6
 ...
 200  continue
      ...
      MACTFV(3) = NDIMI
      call SMESS(MACTFV, MTXTAG, INTCHK, X)
      MACTFV(3) = NTAB(1)
      call SMESS(MACTFV, MTXTAH, INTCHK, XT)
c
...
c  Print XT, YT data for the current selection.
      ...
      MACTFV(3) = I - 1
      INTCHK(1) = I - 1
      call SMESS(MACTFV, MTXTAI, INTCHK, FDAT)
      MACTFV(3) = KNT(I)
      INTCHK(1) = I
      if (LUP(I) .eq. 3) then
        call SMESS(MACT(3), MTXTAJ, INTCHK, XT(LX(I)))
      else
        call SMESS(MACTFV, MTXTAK, INTCHK, XT(LX(I)))
      end if
      call SMESS (MACTFV, MTXTAL, INTCHK, YT(IYT))
      IYT = IYT + MACTFV(3)
 250  continue
...
c More error message processing for errors that would show in silupm.f
 400  continue
