      real             function SSVAL ( K, NC, T, BCOEF, X, IDERIV )
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1994-10-20 SSVAL Krogh  Changes to use M77CON
c>> 1994-09-26 SSVAL CLL Moved "SSVAL = ZERO" to be 1st executable stmt.
...
c  SSVAL [out] Returned value of the spline function or its requested
c        derivative.
c     ------------------------------------------------------------------
c--S replaces "?": ?SVAL, ?SFIND
c     ------------------------------------------------------------------
      integer KMAX
      parameter(KMAX = 20)
      integer IDERIV, IDERP1, IHI, ILO, IMK, IP1
      integer J, JJ, K, KM1, KMIDER, KMJ, LEFTI, MODE, NC
      real             AJ(KMAX), BCOEF(NC)
      real             DP(KMAX), DM(KMAX), FKMJ, T(NC+K), X, ZERO
...
c     ------------------------------------------------------------------
      SSVAL = ZERO
      if(K .gt. KMAX) then
         call IERM1('SSVAL',1,2,'Require KORDER .le. KMAX.',
     *   'KORDER',K,',')
