      SUBROUTINE DNLSGU(N, P, L, ALF, C, Y, DCALCA, DCALCB, INC, IINC,
     1                  IV, LIV, LV, V)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1996-04-27 DNLSGU Krogh  Changes to get desired C prototypes.
C>> 1994-10-20 DNLSGU Krogh  Changes to use M77CON
C>> 1990-07-02 DNLSGU CLL @ JPL
C>> 1990-06-12 CLL @ JPL
C>> 1990-02-20 CLL @ JPL
c--D replaces "?": ?NLSGU,?CALCA,?CALCB,?IVSET,?RNSG
C
...
C T(1)...T(N),   DNLSGU ATTEMPTS TO COMPUTE A LEAST SQUARES FIT
...
C                      OF IV AND V BEFORE CALLING   DNLSGU.
C
...
C
 120  CONTINUE
C%%     (*dcalcb)( n, p, l, alf, &Iv[NFGCAL], &V[da1] );
      CALL DCALCB(N, P, L, ALF, IV(NFGCAL), V(DA1))
      IF (IV(NFGCAL) .EQ. 0) IV(TOOBIG) = 1
      GO TO 100
C
 999  RETURN
C
C  ***  LAST CARD OF   DNLSGU FOLLOWS  ***
      END
