      subroutine DGESLD(A,LDA,N,IPVT,B)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 1994-10-20 DGESLD Krogh  Changes to use M77CON
C>> 1987-08-18 DGESLD Lawson  Initial code.
c--D replaces "?": ?GESLD, ?AXPY
...
C     ------------------------------------------------------------------
c     Subprograms referenced: DAXPY
C     ------------------------------------------------------------------
      integer LDA,N,IPVT(N), K, KB, L, NM1
      double precision A(LDA,N), B(N), T, ZERO
      parameter ( ZERO = 0.D0 )
C     ------------------------------------------------------------------
      NM1 = N - 1
...
C
         DO 40 KB = 1, N
            ...
            IF (A(K,K) .NE. ZERO) THEN
              B(K) = B(K)/A(K,K)
            ELSE
              CALL ERMSG('DGESLD',1,0,'A diagonal element is zero','.')
              RETURN
            END IF
            T = -B(K)
