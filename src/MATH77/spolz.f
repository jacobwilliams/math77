...
C>> 2001-05-25 SPOLZ  Krogh Minor change for making .f90 version.
c>> 1996-04-27 SPOLZ  Krogh Changes to use .C. and C%%.
c>> 1996-03-30 SPOLZ  Krogh Added external statement, MIN0 => MIN.
C>> 1995-01-25 SPOLZ  Krogh Automate C conversion.
C>> 1995-11-17 SPOLZ  Krogh SFTRAN converted to Fortran 77
C>> 1994-10-19 SPOLZ  Krogh  Changes to use M77CON
C>> 1992-05-11 SPOLZ  CLL
C>> 1988-11-16        CLL More editing of spec stmts.
C>> 1988-06-07 SPOLZ  CLL Reordered spec stmts for ANSI standard.
...
c++  Code for COMPLEX is active
      ...
      Z(EN) = Z(NA)
      IF (ZZ .NE. ZERO) Z(EN) = CMPLX(X-W/ZZ,ZERO)
c++  Code for NO_COMPLEX is inactive
C      Z(2*NA-1) = X + ZZ
C      Z(2*NA) = ZERO
C      Z(2*EN-1) = Z(2*NA-1)
C      Z(2*EN) = Z(2*NA)
C      IF (ZZ .NE. ZERO) THEN
C        Z(2*EN-1) = X - W/ZZ
C        Z(2*EN) = ZERO
C      END IF
c++  End
