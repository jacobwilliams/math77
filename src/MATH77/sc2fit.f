      SUBROUTINE SC2FIT(XI,YI,SDI,NXY,B,NB,W,NW,YKNOT,YPKNOT,
     *         SIGFAC, IERR1)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 2000-12-01 SC2FIT Krogh  Dim. SDI(*) instead NXY.
c>> 1995-11-21 SC2FIT Krogh  Converted from SFTRAN to Fortran 77.
c>> 1994-10-19 SC2FIT Krogh  Changes to use M77CON
c>> 1994-01-31 SC2FIT CLL Added test for SDI(i) .le. 0 when SDI(1) > 0.
c>> 1990-01-23 CLL Deleted ref to unused variable NX in call to IERM1
C>> 1989-10-20 CLL
C>> 1987-10-22 SC2FIT Lawson  Initial code.
...
c         SBACC and IERR3 comes from SBSOL.
c
...
c     ------------------------------------------------------------------
c--S replaces "?": ?C2FIT, ?BACC, ?C2BAS, ?ERM1, ?ERV1, ?BSOL, ?TRC2C
...
c     ------------------------------------------------------------------
      ...
      real             XI(NXY), YI(NXY), SDI(*), B(NB), W(NW, 5)
      real             YKNOT(NB), YPKNOT(NB), P(4), ONE, ZERO
      real             DOF, RNORM, sdijp, SIGFAC, WT, WT1
...
C
      IF (NW .lt. NB+4) THEN
        IERR1 = 300
        call IERM1('SC2FIT',IERR1,0,'Require NW .ge. NB+4','NW',NW,',')
        call IERV1('NB',NB,'.')
        go to 300
      END IF
C
...
C                                       TEST SDI(1)
      ...
      ELSE
         IERR1 = 1100
         call ERMSG('SC2FIT',IERR1,0,'Require SD(1) .ne. Zero','.')
         return
      END IF
C
...
c
      IF (B(1) .gt. XI(1) .or. B(NB) .lt. XI(NXY)) THEN
        IERR1 = 500
        call SERM1('SC2FIT',IERR1,0,
     *     'Require B(1) .LE. X(1) and B(NB) .ge. XI(NXY)',
     *     'B(1)',B(1),',')
        call SERV1('X(1)',XI(1),',')
        call SERV1('B(NB)',B(NB),',')
        call SERV1('X(NXY)',XI(NXY),'.')
        go to 300
      END IF
C
...
C
      ...
      DO 120 JPOINT = 1, NXY
         IF( K .gt. NW ) THEN
            call SBACC(W, NW, NBAND, IRNOW, KSIZE, ISEG, JTPREV, IERR2)
            if(IERR2 .ne. 0) then
               IERR1 = 700 + IERR2
               go to 200
            end if
...
c                          Build one equation
         call SC2BAS(XI(JPOINT), 0, ISEG,NEWSEG,B,NB,P)
         IF( USEWT1) THEN
            WT = WT1
         ELSE
            sdijp = sdi(jpoint)
            if(sdijp .gt. ZERO) then
               ...
               IERR1 = 1200
               call ERMSG('SC2FIT',IERR1,0,
     *            'With SD(1) > 0  require all SD(I) > 0.', ',')
               call SERV1('SD(1)',SDI(1),'.')
               call IERV1('I',jpoint,',')
               call SERV1('SD(I)',sdijp,'.')
               return
            end if
         END IF
         DO 100 J = 1,4
...
C
      call SBSOL(1,W, NW, NBAND, IRNOW, JTPREV, W(1,NBAND1),
     *           NPARAM, RNORM, IERR3)
            if(IERR3 .ne. 0) then
               IERR1 = 1000 + IERR2
               call ERMSG('SC2FIT',IERR1,0,
     *         'Singularity noted in SBSOL.','.')
               go to 300
            end if
...
C
      call STRC2C (B,NB,W(1,5),YKNOT,YPKNOT)
      RETURN
c
c             Error in _BACC
  200 call ERMSG('SC2FIT',IERR1,0,
     *   'Error detected in subroutine SBACC','.')
