      subroutine DSFIND ( XT, IX1, IX2, X, LEFTI, MODE )
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1996-03-30 DSFIND Krogh Removed Fortran 90 comment.
c>> 1994-11-11 CLL Replaced Fortran 90 DO WHILE's with GOTO's for F77.
c>> 1994-10-20 DSFIND Krogh  Changes to use M77CON
c>> 1992-11-12 DSFIND C. L. Lawson, JPL
...
c        +1 if                         XT(IX2) < X
c     ------------------------------------------------------------------
c--D replaces "?": ?SFIND, ?ERV1
c     Both use IERM1
c     ------------------------------------------------------------------
      integer IHI, ILO, LEFTI, IX1, IX2, MIDDLE, MODE, STEP
      double precision X,  XT(1:IX2)
c     ------------------------------------------------------------------
...
c                                            Here X .ge. XT(IX2)
            ...
            if(XT(ILO) .ge. XT(IX2)) then
               call IERM1('DSFIND',2,2,'Require T(IX2-1) < T(IX2)',
     *         'IX2',IX2,',')
               call DERV1('T(IX2-1)',XT(IX2-1),',')
               call DERV1('T(IX2)',  XT(IX2),  '.')
            endif
            return
         endif
         ILO = IHI
   ...
   15 continue
         if(ILO .eq. IX1) then
            ...
            if(XT(IX1) .ge. XT(IX1+1)) then
               call IERM1('DSFIND',1,2,'Require T(IX1) < T(IX1+1)',
     *         'IX1',IX1,',')
               call DERV1('T(IX1)',  XT(IX1),  ',')
               call DERV1('T(IX1+1)',XT(IX1+1),'.')
            endif
            return
         endif
         IHI = ILO
