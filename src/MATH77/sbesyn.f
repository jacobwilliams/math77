...
C>> 2009-10-28 SBESYN Krogh/Snyder Moved BGAMSQ = BGAM**2 inside else.
C>> 2000-01-05 SBESYN Krogh Changed c$ comments to c.  (Cray f90 prob.)
C>> 1995-11-13 SBESYN Krogh Converted from SFTRAN to Fortran
...
c--S replaces "?": ?BESYN, ?BESPQ, ?ERV1, ?GAMMA, ?LGAMA
C
...
c     ----------
      ...
      real             LOGPI, LOGTWO, ONE, P, PI, PIV2, PSI1, PSIZ
...
c
      ...
      parameter( C16 = 16.E0, FOUR = 4.0E0)
      parameter(PI = 3.14159265358979323846264338327950288E0)
C
      parameter(HALFPI = 1.57079632679489661923132169163975144E0)
c          LOGPI = ln(pi)
      parameter(LOGPI = 1.14472988584940017414342735135305869E0)
c          C2BYPI = 2/pi
      parameter(C2BYPI = 0.63661977236758134307553505349005744E0)
c          EC = Euler's constant.
      parameter(EC = 0.57721566490153286060651209008240243E0)
c          LOGTWO = Ln(2)
      parameter(LOGTWO = 0.69314718055994530941723212145817657E0)
      ...
      data EPS / ZERO /
C
      data D2SER / +.3674669051966159615185E+00,
     *   -.1782903980807269842231E+01,
     *   +.9411644685122855908427E+00,
     *   -.1958865250248747878077E+01,
     *   +.1557306621108283294475E+01,
     *   -.2521051413546812096437E+01,
     *   +.2184502685635110914145E+01,
     *   -.3140067153452674402872E+01,
     *   +.2817401038921461361158E+01,
     *   -.3772198775799670081858E+01,
     *   +.3452780604492584575060E+01/
C
c     ------------------------------------------------------------------
C
...
c
         do 140 I = 1, NUM
            if ( GNU .eq. ZERO ) then
               ...
               YLOG = SLGAMA(GNU) - GNU * (XLOG-LOGTWO) - LOGPI
