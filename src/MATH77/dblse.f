...
c>> 2001-05-25 DBLSE Krogh Minor change for making .f90 version.
c>> 2001-01-16 DBLSE Krogh  Fixed problem with conversion to C.
c>> 2000-12-01 DBLSE Krogh  Removed unused parameter Factor.
c>> 1999-12-22 DBLSE Krogh  Declared some 'NPR' variables.
c>> 1996-03-30 DBLSE Krogh  Added external statement.
c>> 1994-10-19 DBLSE Krogh  Changes to use M77CON
c>> 1992-10-09 DBLSE CLL  Solve for correction rather than new X.
...
c          of second-level subroutines.  Developing DBLSE from SBLS to
...
c     set S.
c     ------------------------------------------------------------------
c--D replaces "?": ?BLSE, ?ERV1, ?HTCC, ?ROTG, ?ROT, ?DOT
...
c           True means it has.
c     ------------------------------------------------------------------
      external DDOT, DNRM2, DRANU
      double precision DDOT, DNRM2, DRANU
      integer N
      ...
      double precision A(LDA,N), B(MTRUE), X(N), W(N), Z(*), BND(2,N)
      double precision UNBND, SIZE(*), TNORM(N), CC(*), SS(*)
      double precision A1, ABSA1, ALPHA, AMAX
      double precision B1, BNDL, BSAVE, BSIZE, BTEMP
      double precision DIFF(MTRUE), FAC
      double precision HALF, ONE, RANGE, RNORM, RT(MTRUE)
      double precision T, TEMP, TMAX, TOL, TWO, UP, WMAX
      double precision XT(N), XTRY
...
20005 IF (IERR .gt. 0 .or. NSETS .eq. MINMN .or. NF .eq. 0) GO TO 20003
      ...
      IF(X(JF) .ne. ZERO)THEN
               call DAXPY(M-M1, -X(JF), A(M1P1,JF), 1, B(M1P1), 1)
      END IF
20011 CONTINUE
         CMODE = .false.
...
c                                 ****** Error:  IERR = 2 ******
     ...
     *      'Inconsistent setting of bound for X(J).', 'J',J,',')
            call DERV1('UNBND',UNBND,',')
            call DERV1('BND(1,J)',BND(1,J),',')
            call DERV1('BND(2,J)',BND(2,J),',')
      GO TO 31001
      END IF
      END IF
      if(KPRINT .ge. 1)
...
c
      IF(II .gt. NSETS .and. II .le. M1)THEN
            BTEMP = B(II) - DDOT(N, A(II,1), LDA, X, 1)
...
c
               X(JF) = XTRY
      IF(II .ne. NSP1)THEN
                  call DSWAP(N, A(II,1),LDA, A(NSP1,1),LDA)
                  B(II) = B(NSP1)
      END IF
               B(NSP1) = ZERO
      IF(IF0 .ne. IF1)THEN
...
30013 IF(NSP1 .lt. M1)THEN
         call DCOPY(N, A(M1,1), LDA, A(NSP1,1), LDA)
         B(NSP1) = B(M1)
         RT(NSP1) = RT(M1)
      END IF
c     .                              Copy row M to row M1.
c
         call DCOPY(N, A(M,1), LDA, A(M1,1), LDA)
         B(M1) = B(M)
         RT(M1) = RT(M)
...
c
         W(J) = DDOT(LEN, A(NSP1,J), 1, B(NSP1),1)
c
...
c
      call DHTCC(1,NSP1,NSP1+1,M,A(1,JSNEW),  UP,B,M,1)
c
c     .                                  Update pointers.
      INDEX(ISNEW)=INDEX(IF1)
      INDEX(IF1)=JSNEW
...
c
      DO 20161 JF=IF1,IF2
             JJ=INDEX(JF)
             call DHTCC (2,NSETS,NSP1,M,A(1,JSNEW),
     *          UP,A(1,JJ),LDA,1)
20161 CONTINUE
         if(KPRINT .ge. 3)
     *       call DHTCC (2,NSETS,NSP1,M,A(1,JSNEW),  UP,RT(1),M,1)
c
      DO 20164 L=NSP1,M
             A(L,JSNEW)=ZERO
20164 CONTINUE
...
c
      ...
      IF(A(NSP1, JS) .ne. ZERO)THEN
               ...
               call DAXPY(N, FAC, A(NSETS,1), LDA, A(NSP1,1), LDA)
               A(NSP1,JS) = ZERO
...
*           JS=INDEX(IS)
*              call DROTG (A(IS,JS),A(IS+1,JS),CC,SS)
*              A(IS+1,JS)=ZERO
*              LEN = JS-1
*              if(LEN .gt. 0)
*    *            call DROT(LEN,A(IS,1),LDA,A(IS+1,1),LDA,CC,SS)
*              LEN = N - JS
*              if(LEN .gt. 0)
*    *            call DROT(LEN,A(IS,JS+1),LDA,A(IS+1,JS+1),LDA,CC,SS)
*              call DROT (1, B(IS),1, B(IS+1),1, CC,SS)
*              if(KPRINT .ge. 3)
*    *         call DROT (1, RT(IS),1, RT(IS+1),1, CC,SS)
*        end for ! IS
...
c
      IF(L .le. IGIV2)THEN
            call DROTG(A(L,JCOL), A(L+1,JCOL), CC(L), SS(L))
            A(L+1,JCOL) = ZERO
      END IF
20253 CONTINUE
c
...
c++ END
      ...
      IF(abs(A(IS+1,JS)) .gt. abs(A(IS,JS)))THEN
                  call DSWAP(N, A(IS,1),LDA, A(IS+1,1),LDA)
                  TEMP = B(IS)
                  B(IS) = B(IS+1)
                  B(IS+1) = TEMP
                  TEMP = RT(IS)
      ...
      END IF
               ...
               call DAXPY(N, FAC, A(IS,1),LDA, A(IS+1,1),LDA)
               A(IS+1,JS) = ZERO
...
C     procedure( HOUSEHOLDER TRIANGULARIZATION )
c
c     Build new columns of triangular matrix indexed by IPUT1 thru IPUT200137500
c     Last row to be affected is IRLAST.
c
30023 DO 20274 IPUT = IPUT1, IPUT2
         JPUT = INDEX(IPUT)
         call DHTCC(1,IPUT,IPUT+1,IRLAST,A(1,JPUT), UP,B,M,1)
         if(KPRINT .ge. 4) print'(a,2i4/(5x,5g14.6))',
     *      ' Hous Tri.. IPUT,IRLAST,B(IPUT:IRLAST)=',
     *      IPUT,IRLAST,(B(I),I=IPUT,IRLAST)
      DO 20277 I = IPUT+1, IF2
            call DHTCC(2,IPUT,IPUT+1,IRLAST,A(1,JPUT), UP,
     *                A(1,INDEX(I)),LDA,1)
20277 CONTINUE
         if(KPRINT .ge. 3)
     *       call DHTCC (2,IPUT,IPUT+1,IRLAST,A(1,JPUT),  UP,RT(1),M,1)
...
c
30008 BSAVE = B(NSP1)
      B(NSP1) = B(NSP1) - DDOT(N, A(NSP1,1), LDA, X, 1)
      DO 20284 IS = 1,NSETS
         JS = INDEX(IS)
      IF(IS .gt. 1)THEN
            A(NSP1,JS) = A(NSP1,JS) - DDOT(IS-1, Z(1),1, A(1,JS),1)
      END IF
...
20284 CONTINUE
      DO 20289 IF0 = IF1, IF2
         JF = INDEX(IF0)
         A(NSP1,JF) = A(NSP1,JF) - DDOT(NSETS, Z(1),1, A(1,JF),1)
20289 CONTINUE
      GO TO 20065
c     ------------------------------------------------------------------
...
20296 CONTINUE
      ...
      DO 20302 I = IS+1, IF2
               J = INDEX(I)
               TEMP = FAC*A(IS,J)
               call DAXPY(LEN, TEMP, A(M1P1,JS),1, A(M1P1,J),1)
20302 CONTINUE
            TEMP = FAC*B(IS)
            call DAXPY(LEN, TEMP, A(M1P1,JS),1, B(M1P1),1)
            if(KPRINT .ge. 3)
     *      call DAXPY(LEN, FAC * RT(IS), A(M1P1,JS),1, RT(M1P1),1)
...
30014 DO 20309 IIS = NSETS, 1, -1
          if(IIS .ne. NSETS) call DAXPY(IIS, -Z(IIS+1), A(1,JJS),1, Z,1)
          JJS=INDEX(IIS)
...
30004 IF(KPRINT .ge. 3)THEN
      ...
      ELSE
            MDB = M
      END IF
         call DCOPY(MDB, B,1, DIFF,1)
      DO 20323 JD=1,N
            call DAXPY(MDB, X(JD)-XT(JD), A(1,JD),1, DIFF,1)
20323 CONTINUE
         call DAXPY(MDB, -ONE, RT,1, DIFF,1)
