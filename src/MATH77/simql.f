      subroutine SIMQL(A, LDA, N, EVAL, WORK, IERR)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1994-10-20 SIMQL  Krogh  Changes to use M77CON
c>> 1992-04-24 SIMQL  CLL   Minor edits
c>> 1991-10-23 SIMQL  Krogh Initial version, converted from EISPACK.
c
...
c        .......... form shift ..........
            ...
            if (abs(G) .gt. 1.0E0) then
               R = G * sqrt(1.0E0 + 1.0E0 / G ** 2)
            else
               R = sign(sqrt(1.0E0 + G ** 2), G)
            end if
            ...
            do 700 I = M-1, L, -1
               ...
               if (abs(F) .gt. abs(G)) then
                  R = abs(F) * sqrt(1.E0 + (G / F)**2)
               else if (G .ne. 0.0E0) then
                  R = abs(G) * sqrt(1.E0 + (F / G)**2)
               else
