      subroutine IVECPR(V, N, TEXT, LWIDTH, LUNIT        )
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 2000-12-01 IVECPR Krogh Removed unused parameter METXTF.
C>> 1994-11-11 IVECPR Krogh Declared all vars.
c>> 1994-10-20 IVECPR Krogh Added M77CON code.
c>> 1992-05-03 IVECPR Krogh Convert to use MESSFT for Fortran text.
c>> 1992-04-08 IVECPR Krogh Replaced dummy K with MACT in ?MESS call.
c>> 1991-11-22 IVECPR F. Krogh   Initial code
c++I DEFAULT MACT1D=3, TAIL=", LUNIT"
c++  DEFAULT MACT1D=5, TAIL=", LUNIT, NUMDIG"
c++  REPLACE ", LUNIT        " = TAIL
c--I replaces "?": ?VECPR, ?MESS
c
c     *****     Formal Arguments     ***********************************
c
...
c++  Code for {I} is active
      integer MEIVEC
      parameter (MEIVEC=57)
      data MACT1 / MEIVEC, 0, MERET /
      MACT1(2) = max(N, 0)
c++  Code for ~{I} is inactive
C      integer MEFVEC
C      parameter (MEFVEC=61)
C      data MACT1 / METDIG, 0, MEFVEC, 0, MERET /
...
c++  Code for {I} is active
      call MESS(MACT1, TTEXT, V)
c++  Code for ~{I} is inactive
C      call IMESS(MACT1, TTEXT, MACT, V)
c++  End
      if (MACT(1) .lt. 0) then
