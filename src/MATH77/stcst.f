      SUBROUTINE STCST (A, TCS, MODE, M, ND, MS, S)
c>> 1997-03-31 STCST Krogh  Increased KEDIM, more sine table checks.
c>> 1996-01-23 STCST Krogh  Changes to simplify conversion to C.
C>> 1994-11-11 STCST Krogh  Declared all vars.
c>> 1994-10-20 STCST Krogh  Changes to use M77CON
c>> 1989-06-16 STCST FTK Fix error message on MODE, and TCS.
...
c     -----------------------------------------------------------------
      ...
      REAL             FN, FOUR, ONE
      REAL             SPI4, SUM, T, T1, TP, TPI, TS, TS1, TWO
      REAL             WI, WR, ZERO
...
C                               FATAL ERROR, DEFAULT IS TO STOP IN IERM1
         CALL IERM1 ('STCST', 1, 2, 'BAD ND', 'ND', ND, '.')
         MS = -2
         RETURN
      END IF
      MA = 0
...
c transform being computed.
      DO 10 K = 1, NDD
         ...
         ELSE IF (MODE(K:K) .eq. 'S' .or. MODE(K:K) .eq. 's') THEN
            ...
            MSG1(13:13) = MODE(K:K)
            CALL IERM1 ('STCST',2,2, MSG1, 'for K =',  K, '.')
            MS = -2
            return
         END IF
         ...
         if(ITCSK .eq. 0) then
            MSG2(12:12) = TCS(K:K)
            CALL IERM1 ('STCST', 3, 2, MSG2, 'for K =', K, '.')
            return
         endif
         NF(1) = 1
         IF (ITCSK .GE. 2) THEN
...
c              Skip sine table computation if all appears O.K.
               if (MT .le. 0) go to 15
               if (abs(S(NT/2) - SPI4) .le. 1.E-7) go to 15
            end if
         end if
         NEEDST = .true.
         call ERMSG('STCST', 3, 1,
...
c
c                               Fatal error, default is to stop in IERM1
  200 CALL IERM1 ('STCST', 4, 2, 'Require 0 .le. max(M(K)) .le. 31',
     1  'M', MM, '.')
      MS = -2
      RETURN
c
