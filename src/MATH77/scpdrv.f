      SUBROUTINE SCPDRV (C,NC,D,ND)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 1994-11-11 SCPDRV Krogh   Declared all vars.
C>> 1994-10-20 SCPDRV Krogh  Changes to use M77CON
C>> 1987-12-09 SCPDRV Lawson  Initial code.
c--S replaces "?": ?CPDRV
      INTEGER NC, ND, J, K
      REAL             C(*),D(*),D1,D2,FAC,ZERO,HALF,TWO
...
C
      IF (NC .LT. 0) THEN
        CALL IERM1('SCPDRV',1,0,'REQUIRE NC .GE. 0','NC',NC,'.')
      ELSE
        D(1)=C(1)
        D(2)=C(2)
        IF (NC .EQ. 0) THEN
