...
c>> 2004-11-11 SFMIN  Krogh  Removed if (..) C3 = statement doing nil.
c>> 2000-12-01 SFMIN  Krogh  Removed unused parameter ONE.
c>> 1996-03-30 SFMIN  Krogh  Added external statement.
C>> 1994-10-20 SFMIN  Krogh  Changes to use M77CON
C>> 1994-04-20 SFMIN  CLL Edited to make DP & SP files similar.
...
c     will be
c         TOL2 = 2 * abs(X) * sqrt(R1MACH(4)) + (2/3) * C3
c     where C3 = TOLI/3.E0 + R1MACH(4)**2 where TOLI = max(TOL,0.E0).
c
...
c     ------------------------------------------------------------------
      ...
      real             ASAVE, BSAVE
      real             HALF, TWO, THREE, FIVE, ZERO, PT95
      integer MODE, NEXT, IC, NP
      parameter(HALF = 0.5E0, TWO = 2.0E0, THREE = 3.0E0)
...
c     ------------------------------------------------------------------
      ...
      IF (NP .GT. 0) THEN
         NP = NP - 1
         print*, ' In SFMIN: X= ',X,', XORF= ',XORF,', IC= ',IC
      END IF
      if(MODE .eq. 1) then
         go to (301,302), NEXT
      endif
