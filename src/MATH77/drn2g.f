...
c File: DRN2G.for       Ten subrs used by the
c       David Gay & Linda Kaufman nonlinear LS package.
c       Needed for versions that do not allow Bounded variables.
c       DRN2G is called by DNLAFU, DNLAGU, & DRNSG.
c
C>> 2015-07-09 DRN2G Krogh Introduced TP to avoid divide by 0,      
C>> 2000-01-07 DRN2G Krogh  Moved COV1 = IV(COMAT) up in DN2CVP.
C>> 1998-10-29 DRN2G Krogh  Moved external statement up for mangle.
c>> 1996-07-09 DRN2G Krogh  Changes for conversion to C.
c>> 1995-01-26 DRN2G Krogh  Moved formats up for C conversion.
c>> 1994-11-02 DRN2G Krogh  Changes to use M77CON
c>> 1993-03-10 DRN2G CLL Moved stmt NN = ... to follow IF (IV1 ...
c>> 1992-04-27 CLL Comment out unreferenced stmt labels.
c>> 1992-04-13 CLL Change from Hollerith to '...' syntax in formats.
c>> 1990-06-29 CLL Changes to formats in DN2CVP.
c>> 1990-06-12 CLL Revised DRN2G & DG7LIT from DMG 4/19/90
c>> 1990-03-30 CLL JPL
c>> 1990-03-14 CLL JPL
c>> 1990-06-12 CLL
c>> 1990-04-23 CLL (Recent revision by DMG)
*** from netlib, Thu Apr 19 11:58:57 EDT 1990 ***
c--D replaces "?": ?RN2G,?C7VFN,?D7TPR,?D7UPD,?G7LIT,?ITSUM,?IVSET
...
C  ***  LAST LINE OF  DRN2G FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE DG7LIT(D, GG, IV, LIV, LV, P, PS, V, X, YY)
...
C
      INTEGER LIV, LV, P, PS
      INTEGER IV(LIV)
      DOUBLE PRECISION D(P), GG(P), V(LV), X(P), YY(P)
C
C -------------------------  PARAMETER USAGE  --------------------------
C
C D.... SCALE VECTOR.
...
C             PUTING GG, HC, AND YY THE NEXT TIME DG7LIT RETURNS WITH
...
c     integer DUMMY
     ...
     2        TEMP1, TEMP2, W1, X01
      DOUBLE PRECISION E, STTSST, T, T1, TP
C
C     ***  CONSTANTS  ***
C
...
c     LOGICAL STOPX
     ...
     2         DR7MDC, DS7LUP, DS7LVM,        DV2AXY,DV7CPY, DV7SCP,
     3         DV2NRM
...
C
 110  CALL DITSUM(D, GG, IV, LIV, LV, P, V, X)
 120  K = IV(NITER)
...
C
         ...
         IPIV1 = IV(IPIVOT)
         CALL DL7MST(D, GG, IV(IERR), IV(IPIV1), IV(KALM), P, V(QTR1),
...
C
      E = V(PREDUC) - V(FDIF)
      S1 = IV(S)
      CALL DS7LVM(PS, YY, V(S1), V(STEP1))
...
C
 470  CALL DV2AXY(PS, YY, NEGONE, YY, GG)
C
...
C
 490  CALL DV2AXY(PS, V(G01), ONE, YY, V(G01))
C
C  ***  UPDATE S  ***
C
...
C
 520  IV(RESTOR) = 0
 530  CALL DF7HES(D, GG, I, IV, LIV, LV, P, V, X)
      GO TO (540, 550, 580), I
...
C
 10   CALL DV7SCL(LH, V(COV), V(F)/(HALF * dble(max(1,N-P))), V(COV))
      IV(COVMAT) = COV
C
 999  RETURN
C  ***  LAST LINE OF DC7VFN FOLLOWS  ***
      END
      SUBROUTINE DF7HES(D, GG, IRT, IV, LIV, LV, P, V, X)
C
...
C
      ...
      INTEGER IV(LIV)
      DOUBLE PRECISION D(P), GG(P), V(LV), X(P)
C
C  ***  LOCAL VARIABLES  ***
C
...
C
 999  RETURN
C  ***  LAST CARD OF DN2RDP FOLLOWS  ***
      END
      SUBROUTINE DO7PRD(L, LS, PP, SS, WW, YY, Z)
C
...
c     ------------------------------------------------------------------
      INTEGER L, LS, PP
      DOUBLE PRECISION SS(LS), WW(L), YY(PP,L), Z(PP,L)
