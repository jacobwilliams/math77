...
c>> 1996-03-30 CGEFA  Krogh   Added external statement.
C>> 1987-08-14 CGEFA  Lawson  Initial code.
C
C     CGEFA computes the LU factorization of the N x N matrix A by
...
C
C     CGEFA may be referenced indirectly via _GECO, but it can be called
C     directly with a saving in time if the reciprocal condition number
C     RCOND is not needed.
C     (Time for _GECO) = (1 + 9/N)*(Time for CGEFA) .
C
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
C     Subprograms referenced: CAXPY,CSCAL,ICAMAX
c     ------------------------------------------------------------------
      external ICAMAX
      integer ICAMAX, J, K, KP1, L, NM1
      integer LDA, N, IPVT(N), INFO
      complex A(LDA,N), T, CZERO, CONE
