      SUBROUTINE  SNLSFB(N, P, L, ALF, B, C, Y, SCALCA, INC, IINC, IV,
     1                  LIV, LV, V)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>> 1996-04-27 SNLSFB Krogh  Changes to get desired C prototypes.
c>> 1994-10-20 SNLSFB Krogh  Changes to use M77CON
c>> 1990-07-02 SNLSFB CLL @ JPL
c>> 1990-06-12 CLL @ JPL
c>> 1990-02-14 CLL @ JPL
*** from netlib, Fri Feb  9 13:10:09 EST 1990 ***
c--S replaces "?": ?NLSFB,?CALCA,?IVSET,?RNSGB,?V2AXY,?V7CPY,?V7SCL
C
...
C  SCALCA (IN)  SUBROUTINE TO COMPUTE A MATRIX.
...
c                 then LV can be 4*N less than just described.
...
C
      EXTERNAL SIVSET, IDSM, SRNSGB,SV2AXY,SV7CPY, SV7SCL
C
C SIVSET.... PROVIDES DEFAULT IV AND V VALUES.
C IDSM...... DETERMINES EFFICIENT ORDER FOR FINITE DIFFERENCES.
C SRNSGB... CARRIES OUT NL2SOL ALGORITHM.
C SV2AXY.... ADDS A MULTIPLE OF ONE VECTOR TO ANOTHER.
C SV7CPY.... COPIES ONE VECTOR TO ANOTHER.
C SV7SCL... SCALES AND COPIES ONE VECTOR TO ANOTHER.
C
C  ***  LOCAL VARIABLES  ***
C
      LOGICAL PARTJ
...
C
      INTEGER AMAT, COVREQ, D, DAMAT, DLTFDJ, GPTR, GRP, IN, IVNEED, J,
     1        L1SAV, MAXGRP, MODE, MSAVE, NEXTIV, NEXTV, NFCALL, NFGCAL,
     2        PERM, R, RESTOR, TOOBIG, VNEED, XSAVE
C
C  ***  IV SUBSCRIPT VALUES  ***
C
C/6
C     DATA AMAT/113/, COVREQ/15/, D/27/, DAMAT/114/, DLTFDJ/43/,
C    1     GPTR/117/, GRP/118/, IN/112/, IVNEED/3/, J/70/, L1SAV/111/,
C    2     MAXGRP/116/, MODE/35/, MSAVE/115/, NEXTIV/46/, NEXTV/47/,
C    3     NFCALL/6/, NFGCAL/7/, PERM/58/, R/61/, RESTOR/9/, TOOBIG/2/,
C    4     VNEED/4/, XSAVE/119/
C/7
      PARAMETER (AMAT=113, COVREQ=15, D=27, DAMAT=114, DLTFDJ=43,
     1           GPTR=117, GRP=118, IN=112, IVNEED=3, J=70, L1SAV=111,
     2           MAXGRP=116, MODE=35, MSAVE=115, NEXTIV=46, NEXTV=47,
     3           NFCALL=6, NFGCAL=7, PERM=58, R=61, RESTOR=9, TOOBIG=2,
     4           VNEED=4, XSAVE=119)
C/
...
C
 50   CALL SRNSGB(V, ALF, B, C, V, IV, IV, L, 1, N, LIV, LV, N, M, P, V,
     1            Y)
...
C
 ...
 70      CONTINUE
      ...
      IPNTR1 = BWA1 + P
      JPNTR1 = IPNTR1 + L + 2
      CALL IDSM(LP1, P, M, IV(IN1), IV(JN1), IV(NGRP1), NG, K, I,
...
C
 130  CALL SRNSGB(V(A1), ALF, B, C, V(DA1), IV(IN1), IV, L, L1, N, LIV,
     1            LV, N, L1, P, V, Y)
...
C
 140  NF = IV(NFCALL)
C%%     (*scalca)( n, p, l, alf, &nf, &V[a1] );
      CALL SCALCA(N, P, L, ALF, NF, V(A1))
      IF (NF .LE. 0) IV(TOOBIG) = 1
      IF (L1 .LE. L) GO TO 130
      IF (IV(RESTOR) .EQ. 2) CALL SV7CPY(N, V(RSAVE0), V(RSAVE1))
      CALL SV7CPY(N, V(RSAVE1), V(ALP1))
      GO TO 130
C
C  ***  COMPUTE DR = GRADIENT OF R COMPONENTS  ***
C
 150  IF (L1 .GT. L .AND. IV(NFGCAL) .EQ. IV(NFCALL))
     1      CALL SV7CPY(N, V(RSAVE0), V(RSAVE1))
      GPTR1 = IV(GPTR)
      DO 260 K = 1, NG
         CALL SV7CPY(P, V(XSAVE1), ALF)
         GRP1 = IV(GPTR1)
         GRP2 = IV(GPTR1+1) - 1
         GPTR1 = GPTR1 + 1
         DO 180 I1 = GRP1, GRP2
 ...
 170        X0I = XSAVE0 + I
            V(X0I) = XI1
 180        CONTINUE
C%%      (*scalca)( n, p, l, &V[xsave1], &nf, &V[da1] );
         CALL SCALCA(N, P, L, V(XSAVE1), NF, V(DA1))
...
C                 *** FULL FINITE DIFFERENCE FOR COV. AND REG. DIAG. ***
                  AJ = A0 + J1*N
                  CALL SV2AXY(N, V(DAJ), NEGONE, V(AJ), V(DAJ))
                  GO TO 230
 220           IF (J1 .GT. L)
     1            CALL SV2AXY(N, V(DAJ), NEGONE, V(RSAVE0), V(DAJ))
 230           CALL SV7SCL(N, V(DAJ), H, V(DAJ))
 240           CONTINUE
 250        CONTINUE
         IF (K .GE. NG) GO TO 270
         IV(1) = -2
         CALL SRNSGB(V(A1), ALF, B, C, V(DA1), IV(IN1), IV, L, L1, N,
     1               LIV, LV, N, L1, P, V, Y)
