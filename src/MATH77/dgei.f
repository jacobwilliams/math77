      subroutine DGEI(A,LDA,N,IPVT,WORK)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
C>> 1994-10-20 DGEI   Krogh  Changes to use M77CON
C>> 1987-08-18 DGEI   Lawson  Initial code.
c--D replaces "?": ?GEI, ?AXPY, ?SCAL, ?SWAP
C
...
c     ------------------------------------------------------------------
c     Subprograms referenced: DAXPY, DSCAL, DSWAP
C     ------------------------------------------------------------------
      integer LDA,N,IPVT(N), I, J, K, KB, KP1, L, NM1
      double precision A(LDA,N), WORK(N), T, ONE, ZERO
...
C
         DO 100 K = 1, N
            IF (A(K,K) .NE. ZERO) THEN
              A(K,K) = ONE/A(K,K)
            ELSE
              CALL ERMSG('DGEI',1,0,'A diagonal element is zero','.')
              RETURN
            END IF
            T = -A(K,K)
