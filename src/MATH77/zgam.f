...
c>> 1996-03-30 ZGAM Krogh Added external statement.
C>> 1995-11-20 ZGAM Krogh Set up so M77CON converts between "Z" and "C".
C>> 1994-08-17 CLL Add tests on BIGINT to allow easier conversion to C.
C>> 1994-05-25 ZGAM WVS generate COEF using PARAMETER
C>> 1994-04-20 ZGAM CLL Make DP and SP versions similar.
C>> 1993-04-13 ZGAM CLL Edit for conversion to C.
C>> 1992-04-20 ZGAM CLL Edited comments.
C>> 1991-11-11 ZGAM CLL Made [Z/C]GAM from CDLGAM
...
c     -----------------------------------------------------------------
c--Z replaces "?": ?GAM
c--D (type)replaces "?": ?ERM1, ?ERV1
c     Also uses I1MACH, and D1MACH
c     -----------------------------------------------------------------
      external D1MACH, I1MACH
      double precision D1MACH
      double precision A,AL1,AL2,AL2P,B,BIGINT
      double precision CARG(2),CANS(2),COEF(7), CUT1
      double precision DE0,DE1,DELTA,DESET,DN
      double precision ELIMIT, EPS3, EPS4,ERREST,F0,F1,G0,G1
      double precision H,H1,H2,HL2P,OMEGA, ONE
      double precision PI, REPS3, T1,T2,TWOPI
      double precision U,U1,U2,UU1,UU2,UUU1,UUU2,V1,V2,VV1,VV2
      double precision W1,W2,W3,Y1,Z1,Z2, ZERO, ZZ1
...
C
      ...
      parameter(F0 = 840.07385296052619D0, F1 = 20.001230821894200D0)
      parameter(G0 = 1680.1477059210524D0, G1 = 180.01477047052042D0)
      parameter(PI = 3.14159265358979323846D0)
      parameter(TWOPI = 6.283185307179586476925D0)
      parameter(HL2P = 0.918938533204672742D0)
      parameter(AL2P = 1.83787706640934548D0)
...
c
      if(Z1 .lt. -BIGINT) then
         CALL DERM1('ZGAM',3,0,'Require CARG(1) .ge. -BIGINT',
     *              'CARG(1)', Z1, ',')
...
C
  100 LF3 = 0
      ...
      W2 = ZERO
      K  = 0
      B  = max(0.1D0, min(10.0D0, 14.142D0-Z2)) - Z1
...
C FINAL ASSEMBLY
      ...
      IF (MODE .EQ. 0 .or. H .le. 1.0D-2) then
         A  = log(H)*0.5D0
