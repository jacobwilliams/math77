      subroutine dspge(N, ISPEC, IA, A, B, OPT)
c Time-stamp: <2015-07-08 15:34:21 m>
c Copyright (c) 2006, Math a la Carte, Inc.
 
c Solve the system of linear equations, A X = B, return X in B.
c>> 2015-07-08 DSPGE Krogh -- Probably this code is worthless      
c>> 2006-03-12 DSPGE    Krogh Initial code
 
c--D replaces "?": ?SPGE, ?MESS
...
c ************************** Declarations ******************************
     ...
     3  M, MROWSZ, NB,  NI, NP
      double precision  PVAL, TP, TP1, TP2, TP3
...
cAK A bug in DSPGE -- Could not find unused column.$E
cAL A bug in DSPGE -- Pivot row not found where expected.$E
c   $
cAM $I $I $I;  $B
c   $
cAN $I.$N$B
     ...
     * LTXTAK=521,LTXTAL=570,LTXTAM=  1,LTXTAN=  1)
      ...
      data MTXTAA/'DSPGE$BN = $I, IA dim. = $I, A dim. = $I, Options: $E
     ...
     *al [1,N].$EA bug in DSPGE -- Could not find unused column.$EA bug$
     * in DSPGE -- Pivot row not found where expected.$E  '/
      data MTXTAB/'$I $I $I;  $B'/
      data MTXTAC/'$I.$N$B'/
...
C      IDAT(1) = NI
C      IDAT(2) = LCLISC - NI
C      FDAT(1) = dble(IDAT(2)) / dble(NI*NI)
C      call DMESS(MACTXX, PROB, IDAT, FDAT)
c++  End
...
C        MACTSP(4) = IA(K)
C        MACTSP(9) = IA(K)
C        call DMESS(MACTSP, SPCOL, IA(K+1), A(K+1))
C        K = K + IA(K) + 1
C 380  continue
c++  End
c++  Code for DEBUG > 7 is inactive
Cc              Print the initial B
C      call DMESS(MACTFV, BVEC, IDAT, B)
c++  End
c++  Code for DEBUG > 4 is inactive
Cc              Print the scaling
C      call DMESS(MACTSV,COLSCA, IDAT, A(LCLISC+1))
C      call DMESS(MACTSV,ROWSCA, IDAT, A(LRSWSC+1))
c++  End
c++  Code for DEBUG > 4 is inactive
...
C        MACTSP(4) = IA(LR)
C        MACTSP(9) = IA(LR)
C        call DMESS(MACTSP, SPROW, IA(LR+1), A(LR+1))
c++  End
        TP = 1.D30
        KB = 0
        J = IA(LR)
...
c++  End
          ...
          else if (abs(A(K)) * TP .gt. dble(KK)) then
            TP = dble(KK) / abs(A(K))
            KB = K
          end if
...
C      MACTSP(4) = IA(LCP)
C      MACTSP(9) = IA(LCP)
C      call DMESS(MACTSP, SPCOL, IA(LCP+1), A(LCP+1))
c++  End
...
c Copy U data to factored space and select the new pivot.
          if (A(LCFW+I) .ne. 0.D0) then
            K = K + 1
            IA(K) = I
            A(K) = A(LCFW+I)
            A(LCFW+I) = 0.D0
            TP1 = (dble(IA(LROWSZ+IA(LROWLI+I))) + .125D0)**2
...
C        FDAT(1) = 1.D0 / A(IA(LNEXT+2))
C        call DMESS(MACTXX, COLU,IDAT, FDAT)
C        call MESS(MACTIV, ROWSZ, IA(LROWSZ+1))
...
C        MACTSP(9) = MACTSP(4)
C        call DMESS(MACTSP, SPCOL, IA(LNEXT+3), A(LNEXT+3))
...
c                Row swaps are easier
 ...
 1410   continue
        do 1440 J = 1, NI
          ...
          if (abs(TP) .gt. 1.D25) then
            TP = TP * 1.D-25
            OPT(3) = OPT(3) - 25.D0
          else if (abs(TP) .lt. 1.D-25) then
            TP = TP * 1.D25
            OPT(3) = OPT(3) + 25.D0
          end if
 1440   continue
...
c              Output of options
 ...
 2020 continue
      ...
      call DMESS(MACTTX, MTXTAA, IDAT, FDAT)
      ISPEC(1) = -MACTER(3)
      return
      end
